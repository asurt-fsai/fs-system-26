...
bool asurt_msgs__msg__bounding_boxes__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("asurt_msgs.msg._bounding_boxes.BoundingBoxes", full_classname_dest, 44) == 0);
  }
  asurt_msgs__msg__BoundingBoxes * ros_message = _ros_message;
  {  // view_id
  ...
  {  // bounding_boxes
    ...
    if (!asurt_msgs__msg__BoundingBox__Sequence__init(&(ros_message->bounding_boxes), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create asurt_msgs__msg__BoundingBox__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    asurt_msgs__msg__BoundingBox * dest = ros_message->bounding_boxes.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      if (!asurt_msgs__msg__bounding_box__convert_from_py(PySequence_Fast_GET_ITEM(seq_field, i), &dest[i])) {
        Py_DECREF(seq_field);
        Py_DECREF(field);
        return false;
      }
...
PyObject * asurt_msgs__msg__bounding_boxes__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("asurt_msgs.msg._bounding_boxes");
  ...
  {  // bounding_boxes
    ...
    for (size_t i = 0; i < size; ++i) {
      ...
      PyObject * pyitem = asurt_msgs__msg__bounding_box__convert_to_py(item);
      if (!pyitem) {
        Py_DECREF(field);
        return NULL;
      }
