...
class VoronoiPlanningNode(Node):
    ...
    def declareParameters(self) -> None:
        ...
        self.declare_parameter("planning.voronoi.max_odom_age_sec", 0.5)
    ...
    def sendToControl(self) -> None:
        ...
        """
        if not self.localization_ready or self.carPosition is None or self.carDirection is None:
        ...
        odom_age_sec = (
            (self.get_clock().now() - self.last_localization_time).nanoseconds / 1e9
            if self.last_localization_time is not None
            else float("inf")
        )
