...
class Zed_to_Landmark(Node):
    def __init__(self):
        ...
        self.status = StatusPublisher("/status/perception_camera_node", self)
        ...
        self.orange_gate_pub = self.create_publisher(
            Bool,
            '/perception/isOrangeCone',
            10
        )

        self.time_duration = self.create_publisher(
            Float64,
            '/diagnostics/comp_time/perception',
            10
        )
    ...
    def listener_callback(self, msg):
        ...
        if detected and not self.orange_gate_last_seen:
            ...
            self.last_gate_seen_distance = self.travelled_distance
        ...
        self.time_duration.publish(Float64(data=elapsed_time))
    ...
    def detect_orange_gate(self, cone_array):
        ...
        if self.last_gate_seen_distance is not None:
            ...
            if distance_since_last_gate < 20.0:
                return False
        
        for cone in cone_array.landmarks:

            if (cone.type != cone.LARGE_CONE) and (cone.type != cone.ORANGE_CONE):
                continue
