...
from std_msgs.msg import Bool, Float32, Float64, Int16, String
...
class CommunicationLayer(rclpy.node.Node):
    ...
    def __init__(self):
        ...
        node_name = os.getenv("SUPERVISOR_NODE_NAME", "communication_layer")
        ...
        self._module_heartbeat_subscriptions = {}
    ...
    def _setup_publishers(self):
        ...
        self._mission_flag_pub = self.create_publisher(Bool,"/ros_can/mission_completed",10)
        self._mission_state_pub = self.create_publisher(String, "/static_mission/state", 10)
        self._module_heartbeat_pub = self.create_publisher(NodeStatus, "/module_heartbeat", 10)
    ...
    def _setup_subscriptions(self):
        ...
        self._module_heartbeat_sub = self.create_subscription(NodeStatus, "/module_heartbeat", self.onHeartbeat, 10)

        self.create_subscription(TwistWithCovarianceStamped,'/ros_can/twist', self.onVelocity,10)
        
        self.create_subscription(AckermannDriveStamped,'/ackr', self.onControl,10)
        # Bool topics
        bool_topics = [
            ("/perception/isOrangeCone", self.onConeDetection),
            ("/loop_closure_flag",         self.onLoopClosure),
        ]
        for topic, callback in bool_topics:
            self.create_subscription(Bool, topic, callback, 10)

        # Float64 topics
        self.create_subscription(Float32, "/slam/distance", self.onDistance, 10)
           
        self.create_subscription(Int16, "/supervisor/loopClosureCount",self.onLoopClosureCount,10)
    ...
    def _setup_timers(self):
        ...
        # Mission tick — drives StaticA/B/AutoDemo step machines at 20 Hz
    ...
    def _mission_tick(self):
        mission = self._activeMission
        if mission is None:
            return
        try:
            if hasattr(mission, "tick"):
                mission.tick()
        except Exception as e:
            self.logger.error(f"[Mission Tick Error] {e}", exc_info=True)
    ...
    def registerModuleHeartbeats(self, modules):
        """
        Register heartbeat subscriptions for the current mission modules.

        Input
        -----
        modules : list[Module]
        """
        for topic, sub in self._module_heartbeat_subscriptions.items():
            try:
                self.destroy_subscription(sub)
            except Exception as exc:
                self.logger.warning(
                    f"[Heartbeat] Failed to remove subscription topic={topic}: {exc}"
                )
        self._module_heartbeat_subscriptions = {}

        if not modules:
            self.logger.info("[Heartbeat] Cleared module heartbeat subscriptions")
            return

        for module in modules:
            topic = getattr(module, "heartbeatTopic", None)
            if not topic:
                self.logger.warning(
                    f"[Heartbeat] Module {module.pkg} missing heartbeats_topic; skipping"
                )
                continue
            if topic == "/module_heartbeat":
                continue
            if topic in self._module_heartbeat_subscriptions:
                self.logger.warning(
                    f"[Heartbeat] Duplicate heartbeat topic {topic} for module {module.pkg}; skipping"
                )
                continue
            self.logger.info(f"[Heartbeat] Subscribing to {topic} for module {module.pkg}")
            sub = self.create_subscription(
                NodeStatus,
                topic,
                lambda msg, pkg=module.pkg: self._relay_module_heartbeat(msg, pkg),
                10,
            )
            self._module_heartbeat_subscriptions[topic] = sub

    def _relay_module_heartbeat(self, msg: NodeStatus, module_name: str):
        if not module_name:
            module_name = msg.message
        if not module_name:
            self.logger.warning("[Heartbeat] Received heartbeat without module name")
            return
        relay_msg = NodeStatus()
        relay_msg.header = msg.header
        relay_msg.status = msg.status
        relay_msg.message = module_name
        self._module_heartbeat_pub.publish(relay_msg)
    ...
    def onCANState(self, msg):
        self.logger.info(f"[FLOW] CommunicationLayer.onCANState entered: as_state={msg.as_state} ami_state={msg.ami_state}")
        """
    ...
    def onVelocity(self, msg):
        ...
        self._log_topic("/ros_can/twist", f"{velocity:.2f} m/s")
    ...
    def onHeartbeat(self, msg):
        ...
        status = msg.status
        if not module_name:
            self.logger.warning("[Heartbeat] Received /module_heartbeat without module name")
            return

        self._log_topic("/module_heartbeat", f"{module_name} - {status}")

        if self._supervisor:
            self._supervisor.onHeartbeat(module_name, status)
    ...
    def onConeDetection(self, msg):
        ...
        self._log_topic("/perception/isOrangeCone", msg.data)
    ...
    def onLoopClosure(self, msg):
        ...
        self._log_topic("/loop_closure_flag", msg.data)
    ...
    def onLoopClosureCount(self, msg):
        ...
        """
        self._log_topic("/supervisor/loopClosureCount", msg.data)
    ...
    def publishDriveCommand(self, cmd_msg):
        ...
        self._log_publish("drive_command ana geett henaaaa!!!!!", f"speed={cmd_msg.drive.speed:.2f}, steer={cmd_msg.drive.steering_angle:.2f}")
