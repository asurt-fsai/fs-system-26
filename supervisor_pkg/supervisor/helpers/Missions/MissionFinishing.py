...
class MissionFinishing(ABC):
    ...
    def publishMissionState(self, state: str):
        self.communication.publishMissionState(state)
        self.logger.info(f"[MissionState] {state}")

    def notifyMissionFinished(self):
        self.logger.info(f"[FLOW] notifyMissionFinished entered: current status={self.missionStatus}")
        """
        Input  : None
        Output : None
        ...
        self.publishMissionState("FINISHED")
        if self.supervisor:
