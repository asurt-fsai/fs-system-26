...
class GlobalPlanningNode(Node):
    ...
    def initPubAndSub(self) -> None:
        ...
        # Subscribe to SLAM Map (LandmarkArray)
        self.subscriber1 = self.create_subscription(MarkerArray, conesTopic, self.receiveFromSLAM, 10)
    ...
    def receiveFromSLAM(self, msg: MarkerArray) -> None:
        ...
        self.get_logger().info(f"Received {len(msg.markers)} filtered cones from SLAM Map.")

        self.sendToControl()

    # def process_marker_array(self, msg: MarkerArray) -> None:
    #     """Process incoming global cone map from SLAM."""
    #     self.get_logger().info(f"Received {len(msg.markers)} cones from SLAM Map.")
    #     self.cones = [np.zeros((0, 2)) for _ in ConeTypes]
    ...
    #     """Process incoming global cone map from SLAM using LandmarkArray."""
    #     marker_array = MarkerArray()
    #     for landmark in msg.landmarks:
    #         marker = Marker()
    #         marker.pose.position.x = float(landmark.position.x)
    #         marker.pose.position.y = float(landmark.position.y)
    #         marker.pose.position.z = 0.0

    #         if landmark.type == Landmark.YELLOW_CONE:
    #             marker.color.r = 1.0
    #             marker.color.g = 1.0
    #             marker.color.b = 0.0
    #         elif landmark.type == Landmark.BLUE_CONE:
    #             marker.color.r = 0.0
    #             marker.color.g = 0.0
    #             marker.color.b = 1.0
    #         elif landmark.type == Landmark.ORANGE_CONE or landmark.type == Landmark.LARGE_CONE:
    #             marker.ns = "Large Orange Cone"
            
    #         marker_array.markers.append(marker)

    #     self.process_marker_array(marker_array)
