...
class PlanningNode(Node): # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self) -> None:
        ...
        self.get_logger().info("Path Planner MIX instantiated...")
        ...
        self.status_pub = StatusPublisher("/status/planning_node", self)
    ...
    def filter_and_transform_cones(self, cones, car_pos, car_yaw):
        """
        Filter cones within a pizza slice in front of the car and transform them to the car's local frame.
        Args:
            cones: List of np.array of shape (N,2) for each cone type (global frame)
            car_pos: np.array([x, y]) (global)
            car_yaw: float (global)
        Returns:
            filtered_local_cones: List of np.array of shape (M,2) for each cone type (local frame)
        """
        filtered_local_cones = [np.zeros((0, 2)) for _ in ConeTypes]
        ...
        for cone_type, cone_arr in enumerate(cones):
            ...
            # Rotate so car faces x-axis (local frame)
            c, s = np.cos(-car_yaw), np.sin(-car_yaw)
            rot = np.array([[c, -s], [s, c]])
            rel_local = rel @ rot.T  # shape (N,2)
            # Polar coordinates in local frame
            dists = np.linalg.norm(rel_local, axis=1)
            angles = np.arctan2(rel_local[:, 1], rel_local[:, 0])
            # Filter by radius and angle
            mask = (dists <= self.slice_radius) & (np.abs(angles) <= half_angle)
            filtered_local_cones[cone_type] = rel_local[mask]
        return filtered_local_cones
    ...
    def receiveFromPerception(self, msg: MarkerArray) -> None:
        ...
        for marker in msg.markers:
            ...
            # Include white markers as unknown cones.
        ...
        # === NEW: Filter and transform cones ===
        # Use current car position/direction for filtering
        car_pos = self.carPosition.copy()
        car_yaw = self.carDirection
        filtered_local_cones = self.filter_and_transform_cones(self.cones, car_pos, car_yaw)
        self.filtered_local_cones = filtered_local_cones  # Save for plotting if needed
    ...
    def sendToControl(self) -> None:
        ...
        # === NEW: Always call planner with car at (0,0), heading 0, and filtered cones ===
        path_local = self.pathPlanner.calculatePathInGlobalFrame(
            vehiclePosition=np.array([0, 0]),
            vehicleDirection=0.0,
            cones=self.filtered_local_cones
        )
        self.path = None
        if path_local is not None:
            # === Transform path back to global frame ===
            # c, s = np.cos(self.carDirection), np.sin(self.carDirection)
            # rot = np.array([[c, -s], [s, c]])
            # path_global = (path_local @ rot.T) + self.carPosition  # shape (N,2)
            ...
            # # === Plotting (optional: plot filtered cones in local or global frame) ===
            # self.ax.clear()
            # self.ax.set_title("Path Planning Visualization")
            # self.ax.set_xlabel("X (m)")
            # self.ax.set_ylabel("Y (m)")
            ...
            # # Plot filtered cones (global, for visualization)
            # for cone_type, color, label in [
            #     (ConeTypes.BLUE, 'b*', 'Filtered Blue'),
            #     (ConeTypes.YELLOW, 'y*', 'Filtered Yellow')
            # ]:
            #     local_cones = self.filtered_local_cones[cone_type]
            #     if local_cones.shape[0] > 0:
            #         # Transform back to global for plotting
            #         c, s = np.cos(self.carDirection), np.sin(self.carDirection)
            #         rot = np.array([[c, -s], [s, c]])
            #         global_cones = (local_cones @ rot.T) + self.carPosition
