...
# python-pcl is notoriously difficult to build; the package shipped to
# PyPI sometimes only contains metadata and does not export the classes
# we need.  Importing it at module load time causes an ImportError that
# prevents the entire package from being imported.  Instead, perform a
# lazy import and expose names only if available.  This allows the rest
# of the utilities to be used even when pcl bindings are not installed.
try:
    from pcl import PointCloud, PointCloud_PointXYZI
except ImportError:  # pragma: no cover - pcl is optional
    PointCloud = None  # type: ignore[assignment]
    PointCloud_PointXYZI = None  # type: ignore[assignment]
...
class Utils:
    ...
    def createLandmarkMessage(
        ...
        for cone, typ, prob in zip(cones, types, coneProbs):
            ...
            coneMsg.probability = float(prob)
    ...
    def npToPcl(self, cloudArray: npt.NDArray[np.float64]) -> PointCloud:
        ...
        ########################   This is slow  ##################################
        ###########################################################################
        if PointCloud is None or PointCloud_PointXYZI is None:  # pragma: no cover - cannot test without pcl
            raise RuntimeError(
                "npToPcl requires the python-pcl bindings to be installed. "
                "Install the package with 'pip install python-pcl', or "
                "remove calls to this function if you do not need pcl." 
            )
    ...
    def rosToPcl(self, rosPc2: PointCloud2, squeeze: bool = True) -> PointCloud:
        ...
        # npToPcl already guards for missing bindings
