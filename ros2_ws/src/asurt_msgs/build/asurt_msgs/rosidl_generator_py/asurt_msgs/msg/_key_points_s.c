...
bool asurt_msgs__msg__key_points__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  ...
  {  // keypoints
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      if (!rosidl_runtime_c__uint8__Sequence__init(&(ros_message->keypoints), size)) {
        PyErr_SetString(PyExc_RuntimeError, "unable to create uint8__Sequence ros_message");
        PyBuffer_Release(&view);
        Py_DECREF(field);
        return false;
      }
      uint8_t * dest = ros_message->keypoints.data;
...
PyObject * asurt_msgs__msg__key_points__convert_to_py(void * raw_ros_message)
{
  ...
  {  // keypoints
    ...
    if (ros_message->keypoints.size > 0) {
      ...
      uint8_t * src = &(ros_message->keypoints.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->keypoints.size * sizeof(uint8_t));
      assert(data != NULL);
