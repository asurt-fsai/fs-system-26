...
class VirtualCones:
    def __init__(
        ...
        Args:
            virtual_width       : Default track half-width used when no nearby gate found.
            pairing_threshold   : Max distance to consider a cone as a partner candidate.
            collision_threshold : Min distance a virtual cone must keep from existing cones.
            min_proj            : Rear limit of the forward slice (slightly behind the car).
            max_proj            : Forward limit of the forward slice.
            max_lateral         : Lateral (left/right) limit of the slice.
                                  Set to 8.0 to safely cover the tightest 9m-diameter hairpin.
        """
        self.virtual_width = virtual_width
        self.pairing_threshold = pairing_threshold
        self.collision_threshold = collision_threshold
    ...
    def generate_balanced_cones(self, cone_data, car_pos, car_yaw):
        ...
        Only cones inside the car's local slice window (forward + lateral bounds)
        are considered, so virtual cones are never generated for far-away sections
        that would require a different car yaw to make sense.
        """
        ...
        # --- Separate by color, then slice to the local window ---
        yellows_raw = [c for c in cone_data if c[2] == 'y']
        blues_raw   = [c for c in cone_data if c[2] == 'b']
 
        yellows_sliced = self._get_cones_in_range(yellows_raw, car_pos, car_heading)
        blues_sliced   = self._get_cones_in_range(blues_raw,   car_pos, car_heading)
 
        # Order the sliced walls along the track
        yellows = self._order_cones_along_wall(yellows_sliced, car_pos, car_heading)
        blues   = self._order_cones_along_wall(blues_sliced,   car_pos, car_heading)
 
        # Copy ALL original cones into the output (not just the sliced ones)
        balanced = [(float(c[0]), float(c[1]), c[2], False) for c in cone_data]
 
        midpoint_nodes = []
 
        # Mirror both walls (partner search uses the sliced walls,
        # but collision check uses the full cone_data)
        self._mirror_wall(blues,   yellows, 'y', -1, yellows, blues, balanced, midpoint_nodes, cone_data)
        self._mirror_wall(yellows, blues,   'b',  1, yellows, blues, balanced, midpoint_nodes, cone_data)
    ...
    def _get_cones_in_range(self, cones, car_pos, car_heading):
        """
        Returns only the cones that fall inside the car's local rectangular slice.
 
        The slice is defined in the car's own coordinate frame:
          - Longitudinal axis (heading):   min_proj < projection < max_proj
          - Lateral axis (perpendicular):  |lateral projection| < max_lateral
 
        The lateral bound is crucial on hairpin turns: without it, cones from
        the incoming/outgoing straight can fall inside the forward window even
        though they belong to a different part of the track.
 
        For a 9m-diameter hairpin the outer wall can sit ~7m laterally from
        the inner wall at the apex, so max_lateral=8.0 covers that with margin.
        """
        # Perpendicular to heading (points left of the car)
        perp = np.array([-car_heading[1], car_heading[0]])
 
        result = []
        for c in cones:
            vec     = np.array([c[0], c[1]]) - car_pos
            proj    = np.dot(vec, car_heading)   # forward / backward
            lateral = abs(np.dot(vec, perp))     # left / right
 
            if self.min_proj < proj < self.max_proj and lateral < self.max_lateral:
                result.append(c)
    ...
    def _calculate_virtual_cone(self, p_curr, normal_line, t_color, yellows, blues, cone_data):
        ...
        # ----------------------------------------------------
        # Collision check against ALL real cones (not just sliced)
        # ----------------------------------------------------
    ...
    def _order_cones_along_wall(self, cones, car_pos, car_heading):
        """
        Orders a list of cones by finding the start of the wall and
        ...
        # Find the starting cone: the one furthest back along the heading
        start_idx = min(
            range(len(cones)),
            key=lambda i: np.dot(
                np.array([cones[i][0], cones[i][1]]) - car_pos, heading
            )
        )
        ...
        # Nearest-neighbor walk along the wall
        while remaining:
            last     = np.array([ordered[-1][0], ordered[-1][1]])
            next_idx = min(
                range(len(remaining)),
                key=lambda i: np.linalg.norm(
                    np.array([remaining[i][0], remaining[i][1]]) - last
                )
            )
            ordered.append(remaining.pop(next_idx))
