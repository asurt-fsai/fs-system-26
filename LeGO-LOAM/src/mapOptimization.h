...
class MapOptimization : public rclcpp::Node {
 ...
 private:
  ...
  /////
  int last_loop_closure_keyframe_id_ = 0;
  const int min_keyframe_gap_threshold_ = 40;
  ...
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubOdomAftMappedAdjusted;
  rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pubKeyPoses;
  //new 
  rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr pubLoopDetectedOdom;
  ...
  // Cone map
  pcl::PointCloud<PointType>::Ptr globalConeMap;
  pcl::PointCloud<PointType>::Ptr localConeMap;
  std::deque<pcl::PointCloud<PointType>::Ptr> coneCloudKeyFrames;
  pcl::KdTreeFLANN<PointType> kdtreeConeMap;
  pcl::VoxelGrid<PointType> downSizeFilterCone;
  pcl::PointCloud<PointType>::Ptr currentConeCloud;

  // Parameters (read from ROS params)
  float cone_match_distance_;
  float cone_voxel_size_;
