...
class LidarNode(Node):
    def __init__(self):
        ...
        self.landmark_pub = self.create_publisher(LandmarkArray, '/perception/lidar_landmarks', 10)
    ...
    def callback(self, msg):
        ...
        # The sensor frame is usually velodyne or similar
        ...
        for cluster_id, indices in clusters.items():
            ...
            # Create a Landmark for the cone mapper
            landmark = Landmark()
            landmark.position.x = float(center[0])
            landmark.position.y = float(center[1])
            landmark.position.z = float(center[2])
            landmark.type = 3  # ConeType.UNKNOWN
            landmark.probability = 1.0  # High confidence it's a cone based on classifier
            landmark.identifier = int(cluster_id)
            landmark_array.landmarks.append(landmark)

        if marker_array.markers:
            self.marker_pub.publish(marker_array)
            
        if landmark_array.landmarks:
