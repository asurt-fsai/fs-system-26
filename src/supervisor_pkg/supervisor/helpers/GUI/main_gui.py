...
class StateMachinePanel:
    ...
    def _build_ui(self, parent):
        ...
        self.driving_flag_button = ToggleButton(
            controls_frame,
            text="DRIVING FLAG",
            command=self.toggle_driving_flag,
            width=180,
            height=55
        )
        self.driving_flag_button.grid(row=1, column=1, padx=8, pady=6, sticky="ew")
        ...
        self.ebs_button.grid(row=2, column=1, padx=8, pady=6, sticky="ew")
    ...
    def _schedule_mission_publish(self):
        ...
        timer = threading.Timer(5.0, self._schedule_mission_publish)
        timer.daemon = True
        timer.start()
    ...
    def supervisor_driving_flag_cb(self, msg: Bool):
        if not msg.data:
            return
        self.logger.info("[GUI] /supervisor/driving_flag received: True")
        if self.on_supervisor_driving_flag:
            self.on_supervisor_driving_flag()
    ...
    def toggle_driving_flag(self):
        msg = Bool()
        msg.data = not self.drivingFlag
        self.driving_flag_pub.publish(msg)
        self.drivingFlag = not self.drivingFlag
        self.update()
        self.logger.info(f"[GUI] DRIVING FLAG -> {self.drivingFlag}")
    ...
    def update(self):
        ...
        if self.state == 1:
            ...
            if not self.ebsTimerFlag:
                self._check_timer(5, True, False)
            if not self.ASmaster:
                self.state = 0
                self.can_state_msg.as_state = 0
                self.can_state_pub.publish(self.can_state_msg)
    ...
    def _check_timer(self, duration, ebs, ready):
        ...
        def _timer_done():
            print(str(duration) + " seconds have elapsed.")
