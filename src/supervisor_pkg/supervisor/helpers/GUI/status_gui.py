...
class NodeStatusGUI(Node):
    ...
    def _on_node_double_click(self, event):
        """Show detailed info when double-clicking a node"""
        ...
        if selection:
            ...
            # Details
            details_frame = tk.Frame(popup, bg=CARD, padx=20, pady=20)
            ...
            tk.Label(
                details_frame,
                text=f"Last Heartbeat: {heartbeat}",
                bg=CARD,
                fg=TXT,
                font=("Arial", 11)
            ).pack(anchor="w", pady=5)
            ...
            close_btn = tk.Button(
                ...
                bg=RED_PRIMARY if 'RED_PRIMARY' in globals() else HEADER,
                fg=TXT,
                font=("Arial", 10, "bold"),
                cursor="hand2"
            )
