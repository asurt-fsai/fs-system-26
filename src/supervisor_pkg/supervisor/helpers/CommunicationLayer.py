...
class CommunicationLayer(rclpy.node.Node):
    ...
    def _setup_publishers(self):
        ...
        self._mission_flag_pub = self.create_publisher(Bool,"/ros_can/mission_flag",10) ##to be checked esm el flag
    ...
    def _setup_subscriptions(self):
        ...
        self.create_subscription(TwistWithCovarianceStamped,'/twist', self.onVelocity,10)
        ...
        self.create_subscription(Bool, "/driving_flag", self.onSupervisorDrivingFlag, 10)

        # Mission manager coordination
        self.create_subscription(String, "/mission_manager/modules", self.onMissionModules, 10)
        self.create_subscription(String, "/mission_manager/result", self.onMissionResult, 10)
    ...
    def onSupervisorDrivingFlag(self, msg: Bool):
        self._log_topic("driving_flag", msg.data)
        if self._supervisor:
            self._supervisor.onSupervisorDrivingFlag(msg.data)
    
    
    def onMissionModules(self, msg: String):
        self._log_topic("/mission_manager/modules", "received")
        if self._supervisor:
            self._supervisor.onMissionModules(msg.data)

    def onMissionResult(self, msg: String):
        self._log_topic("/mission_manager/result", msg.data)
        if self._supervisor:
            self._supervisor.onMissionResult(msg.data)
