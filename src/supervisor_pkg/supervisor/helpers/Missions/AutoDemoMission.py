...
class AutoDemoMission(MissionFinishing):
    ...
    def tick(self):
        ...
        if self.step == 0:
            ...
            elapsed = now - self.t0
            self.logger.info(f"[AutoDemo] STEP 1: steering LEFT (elapsed={elapsed:.1f}s, vel=0.0, steer={-self.maxSteer_rad:.3f})")
            ...
            self.publishDrive(0.0, self.maxSteer_rad)
            elapsed = now - self.t0
            self.logger.info(f"[AutoDemo] STEP 2: steering RIGHT (elapsed={elapsed:.1f}s, vel=0.0, steer={self.maxSteer_rad:.3f})")
            ...
            self.logger.info("[AutoDemo] Publishing mission flag = True")
