...
class StaticAMission(MissionFinishing):
    ...
    def tick(self):
        ...
        # -------------------------------------------------
            ...
            self.logger.info(f"[StaticA] STEP 1: steering LEFT (elapsed={elapsed:.1f}s, vel=0.0, steer={-self.maxSteer_rad:.3f})")
        ...
        # -------------------------------------------------
            ...
            self.publishDrive(0.0, self.maxSteer_rad)
            elapsed = now - self.phase_start_time
            self.logger.info(f"[StaticA] STEP 2: steering RIGHT (elapsed={elapsed:.1f}s, vel=0.0, steer={self.maxSteer_rad:.3f})")
