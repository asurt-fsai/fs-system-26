...
class SkidPadPathPlannerNode(Node):  # type: ignore[misc]
    ...
    def __init__(self) -> None:
        ...
        # Subscriptions
        self.create_subscription(MarkerArray, conesTopic, self.listenerCallback, 10)
    ...
    def listenerCallback(self, poseArrayMsg: MarkerArray) -> None:
        """
        ...
        new_cone_positions = np.array(
            [
                [pose.pose.position.x, pose.pose.position.y, pose.type]
                for pose in poseArrayMsg.markers
            ]
        )
