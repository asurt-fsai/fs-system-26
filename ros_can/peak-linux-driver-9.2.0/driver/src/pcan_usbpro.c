...
//#define PCAN_USBPRO_TS_RESYNC_PERIOD	(3600*NSEC_PER_SEC)
...
static int pcan_usbpro_sizeof_rec(u8 data_type)
{
	...
	case DATA_TYPE_USB2CAN_STRUCT_BUSLOAD_RX:
		return sizeof(struct pcan_usbpro_busload_rx);
	...
	case DATA_TYPE_USB2CAN_STRUCT_FKT_SETGET_BUSLOAD_MSG:
		return sizeof(struct pcan_usbpro_busload);
				...
				struct pcan_usbpro_msg *pum)
{
	...
	if (err)
		pr_err(DEVICE_NAME
		       ": err %d submitting cmd to %s\n",
		       err, usb_if->adapter->name);

	return err;
}

/*
 * static int pcan_usbpro_read_rsp(struct pcandev *dev, void *pv, int maxlen)
 *
...
{
	struct pcan_usb_interface *usb_if = pcan_usb_get_if(dev);
	union pcan_usbpro_rec *ps = (union pcan_usbpro_rec *)pv;
	int i, err = 0;

	for (i = 0; i < 3; i++) {
		u8 *pc = dev->port.usb.cout_baddr;
		int r, l, rec_counter;
		...
		if (!l) {
			pr_warn(DEVICE_NAME
				": %s: abnormal null response  (err=%d)."
				" Retrying...\n",
		...
		for (r = 0; r < rec_counter; r++) {
			...
			if (rec_size <= 0) {
				pr_info(DEVICE_NAME
					": %s(): wrong rec_size %d in rsp\n",
					__func__, rec_size);

				dump_mem("unknown rec", pc, l);
				err = -EBADMSG;
				goto fail;
			}

			if (ps->data_type && (pr->data_type == ps->data_type)) {
				...
				case DATA_TYPE_USB2CAN_STRUCT_FKT_GETDEVICENR:
					found_rec = \
						(pr->bus_activity.channel ==
						 ps->bus_activity.channel);
					break;
				default:
					/* no other criteria defined for data
					 * type %02Xh: considering response
					 * received
					 */
					found_rec = 1;
					break;
			...
			if (found_rec) {
				memcpy(pv, pr, min(maxlen, rec_size));
				break;
			}
...
static int pcan_usbpro_set_can_on(struct pcandev *dev)
{
	...
	pcan_usbpro_msg_add_rec(&ub,
	                        DATA_TYPE_USB2CAN_STRUCT_FKT_SETGET_BUSLOAD_MSG,
	                        dev->can_idx,
	                        1);
...
#if defined(DEBUG_TRACE) || defined(DEBUG_CAN_ONOFF)
	pr_info(DEVICE_NAME ": %s(%s CAN%u): tx.frames=%d\n",
		__func__, dev->adapter->name, dev->can_idx+1,
...
/*
 * int pcan_usbpro_calibration_request(struct pcan_usb_interface *usb_if,
 *                                     u16 onoff)
 */
static int pcan_usbpro_calibration_request(struct pcan_usb_interface *usb_if,
					   u16 onoff)
{
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(%s CAN%u, onoff=%d)\n",
		__func__, dev->adapter->name, dev->can_idx+1, onoff);
#endif
...
static int pcan_usbpro_get_device_nr(struct pcandev *dev, u32 *p_device_nr)
{
	...
	if (!err) {
		...
		err = pcan_usbpro_read_rsp(dev, pdn, sizeof(*pdn));
...
static int pcan_usbpro_reset(struct pcandev *dev)
{
	 ...
	 */
	err = pcan_usbpro_open_ex(dev,
				  (struct pcanfd_init *)&dev->init_settings);
	if (err)
		return err;
 ...
 *				    u32 ts_low, struct pcan_timespec *tv)
 */
static struct pcan_timespec *pcan_usbpro_timestamp_decode(struct pcandev *dev,
					u32 ts_low, struct pcan_timespec *tv)
{
	struct pcan_usb_interface *usb_if = pcan_usb_get_if(dev);
	u64 ts_us;
...
/*
 * int pcan_usbpro_return(struct pcandev *dev, int err)
 *
 * Unify the way any PCAN-USB device msg handler should return
 */
int pcan_usbpro_return(struct pcandev *dev, int err)
{
	if (err > 0)
	                                ...
	                                struct pcan_usbpro_canmsg_rx *rx)
{
	...
	f.msg.flags = pcanxl_msg_flags_dlc_set(PCANFD_MSG_STD, rx->len);
	...
	return pcan_usbpro_return(dev, pcan_xxxdev_rx(dev, &f));
}
					...
					u32 ts_low, u32 dev_frame_index)
{
	...
	/* Must consider that timestamps in calibration msgs aren't synchronized
	 ...
	 * It SHOULD be init to some negative value so that the whole
	 * system can initialize too
	 */
	if (usb_if->dev_frame_index < 0) {
		...
		if (unlikely(ts_low <= usb_if->cm_ts_low))
			usb_if->uptime_us += 0x100000000ULL;

		usb_if->uptime_us -= usb_if->cm_ts_low;
		usb_if->uptime_us += ts_low;
	}
	...
	if (usb_if->cm_ignore_count >= 0) {
		...
		dfi = ((dfi >= -2) && (dfi <= 2)) ? dfi * USEC_PER_MSEC : 0;
...
#endif
		pr_info(DEVICE_NAME
			": %s #%u: CM %u %llu %d %d %d %d\n",
			dev->adapter->name, dev->adapter->index,
			ts_low,
			usb_if->uptime_us,
...
/*
 * void pcan_usbpro_init_calibration(struct pcan_usb_interface *usb_if,
 *				     int cm_ign_count)
 *
 * @cm_ign_count > 0	nb of calibration msg to ignore before handling them
 * 		 = 0	clock drift is computed by calibration msg
 * 		 < 0	calibration msg not used to compute clock drift
 */
void pcan_usbpro_init_calibration(struct pcan_usb_interface *usb_if,
				  int cm_ign_count)
{
	usb_if->bus_frame_index = pcan_usbpro_get_frame_number(usb_if);
	usb_if->cm_ignore_count = cm_ign_count;
...
/*
 * int pcan_usbpro_handle_busload_rx(struct pcan_usb_interface *usb_if,
 *                                   struct pcan_usbpro_busload_rx *bl)
 */
static int pcan_usbpro_handle_busload_rx(struct pcan_usb_interface *usb_if,
	                                 struct pcan_usbpro_busload_rx *bl)
{
	...
	pr_info(DEVICE_NAME ": %s(CAN%u): "
	        "bl=[busload_val=0x%04x timestamp=0x%08x]\n",
	        dev->adapter->name, ctrl_index+1,
	        le16_to_cpu(bl->busload_val), le32_to_cpu(bl->timestamp32));
#endif

	return pcan_handle_bus_load(dev,
			(10000UL * le16_to_cpu(bl->busload_val)) >> 12);
}
	                          ...
	                          u8 *msg_addr, int msg_len)
{
	...
	for (rec_count=le16_to_cpu(*usb_msg.u.rec_counter_read); rec_count > 0;
		...
		case DATA_TYPE_USB2CAN_STRUCT_BUSLOAD_RX:
			err = pcan_usbpro_handle_busload_rx(usb_if,
							    &pr->busload_rx);
			break;
	                          ...
	                          u8 *buffer_addr, int *buffer_size)
{
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME
		": %s(buffer_size=%d) rec_max_size=%d fifo_empty=%d\n",
	...
	while (usb_msg.rec_buffer_len < buffer_high_water) {
		...
		err = pcan_txfifo_out(dev, (struct pcanxl_txmsg *)&tx,
				      tx.msg.data);
		if (err < 0)
			break;
		...
		if ((tx.msg.flags & MSGTYPE_RTR) || (dlc == 0)) {
			...
			if (dlc == PCANFD_CAN20_MAXDATALEN) {
				u8 tmp_dlc =
					pcanxl_msg_flags_dlc_get(tx.msg.flags);
				if (tmp_dlc > dlc)
					dlc = tmp_dlc;
			}
		}
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(%s, can_lin=%d, loaded=%d)\n",
	        __func__, usb_if->adapter->name, can_lin,loaded);
#endif
...
int pcan_usbpro_init(struct pcan_usb_interface *usb_if)
{
	...
	pcan_usbpro_init_calibration(usb_if, 3);
