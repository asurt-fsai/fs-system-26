...
int pcan_bitrate_to_bittiming_ex(struct pcan_bittiming *pbt,
				 const struct pcanfd_bittiming_range *caps,
				 u32 sysclock_Hz, int pwm_enconding,
				 int desired_brp);

static inline int pcan_bitrate_to_bittiming(struct pcan_bittiming *pbt,
				const struct pcanfd_bittiming_range *caps,
				u32 sysclock_Hz)
{
	return pcan_bitrate_to_bittiming_ex(pbt, caps, sysclock_Hz, 0, 0);
}

static inline int pcan_is_bittiming_valid(struct pcan_bittiming *pbt)
{
	return (pbt->brp && pbt->tseg1 && pbt->tseg2) || pbt->bitrate;
}
