...
/* hw_type */
enum {
	PCAN_MCU_USBFD_NXP = 1,
	PCAN_MCU_USBPROFD_LPC1778 = 1,

	PCAN_MCU_USBPROFD_LPC4078 = 2,

	PCAN_MCU_USBFD_ATSAM = 10,
	PCAN_MCU_USBPROFD_STM32F733 = 10,
};

#define PCAN_USBFD_VENDOR_INFO_SIZE_V1		28
#define PCAN_USBFD_VENDOR_INFO_SIZE_V234	36
#define PCAN_USBFD_VENDOR_INFO_SIZE_V5		84
...
struct pcan_usbfd_fw_info {
	...
	__le32	ser_no;		/* S/N */
	__le32	flags;		/* special functions (sizeof(this) = 28) */

	/* v2 (ATSAM MCU for example) */
	...
	/* v3 */
	u8	can_bus_count;	/* number of CAN */

	/* v4 */
	u8	instance_idx;	/* USB-X6 instance index */
	u8	dummy;		/* sizeof(this) = 36 */

	/* v5 */
	u8	guid[16];		/* GUID */
	u8	fw_version_adv[32];	/* sizeof(this) = 84 */
...
#define PCAN_USB_FD_SYNC_PERIOD		NSEC_PER_SEC

/* copied from pcan_usbpro_timestamp_decode() but use real hw 64 bit value */
static struct pcan_timespec *_pcan_usbfd_ts_decoder(struct pcandev *dev,
						    ...
						    struct pcan_timespec *tv)
{
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME "%d: %s(ts=%u:%u)\n",
		dev->nMinor, __func__, ts_high, ts_low);
#endif
	...
	if (pcan_usb_get_if(dev)->cm_ignore_count < 0) {
		...
		u64 delta_ns = (ts_us * NSEC_PER_USEC) - dev->time_sync.ts_ns;

		/* simulate sync every PCAN_USB_FD_SYNC_PERIOD */
		if (delta_ns >= PCAN_USB_FD_SYNC_PERIOD) {
...
/* wrapper (SHOULD merge in the future) */
static struct pcan_timespec *pcan_usbfd_ts_decoder(struct pcandev *dev,
						   void *rx_msg,
						   struct pcan_timespec *tv)
{
	struct canfd_msg *canfd_msg = (struct canfd_msg *)rx_msg;
	return _pcan_usbfd_ts_decoder(dev, canfd_msg->ts_low,
				      canfd_msg->ts_high, tv);
}
				       ...
				       int *rsp_size)
{
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(pcan%u): ->EP#%02X\n",
		__func__, dev->nMinor, usb_if->pipe_cmd_out.ucNumber);
#endif
	...
	if (dev->ucan.cmd_len < dev->ucan.cmd_size) {
		/* "All 64 bits should be set to '1' !" */
		__u64 *eoc = canfd_add_cmd(dev, CANFD_CMD_END_OF_COLLECTION);
		*eoc = (__u64)-1;
	}
...
/* int send_cmd_set_can_led(struct pcandev *dev, u8 mode )
 */
static int send_cmd_set_can_led(struct pcandev *dev, u8 mode)
{
	struct ucan_usb_led *cmd;

#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(pcan%u, mode=%u)\n",
		__func__, dev->nMinor, mode);
#endif
	cmd = canfd_add_cmd(canfd_init_cmd(dev), CANFD_USB_CMD_LED_SET);
	if (cmd)
		cmd->mode = mode;
...
/* int send_cmd_set_en_option(struct pcandev *dev, u16 mask, u16 usb_mask)
 */
static int send_cmd_set_en_option(struct pcandev *dev, u16 mask, u16 usb_mask)
{
	struct ucan_usb_option *cmd;

#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(pcan%u, mask=%04xh, usb_mask=%04xh)\n",
		__func__, dev->nMinor, mask, usb_mask);
#endif
	cmd = canfd_add_cmd_set_en_option(canfd_init_cmd(dev), mask, usb_mask);

	/* send the command */
	return canfd_flush_cmd(dev);
}

/* int send_cmd_clr_dis_option(struct pcandev *dev, u16 mask, u16 usb_mask)
 */
static int send_cmd_clr_dis_option(struct pcandev *dev, u16 mask, u16 usb_mask)
{
	struct ucan_usb_option *cmd;
...
#endif
	cmd = canfd_add_cmd_clr_dis_option(canfd_init_cmd(dev), mask, usb_mask);
...
static int pcan_usbfd_set_device_nr(struct pcandev *dev, u32 device_nr)
{
	...
	cmd = canfd_add_cmd(canfd_init_cmd(dev), CANFD_USB_CMD_DEVID_SET);
	if (cmd)
			   ...
			   int should_print, u32 *bl_version)
{
...
#endif
	pfi = kmalloc(sizeof(struct pcan_usbfd_fw_info), GFP_KERNEL);
	if (!pfi)
		return -ENOMEM;
	...
	if (len >= 0) {
		...
		int i;
		u16 sizeof_rec = le16_to_cpu(pfi->size_of);
		u16 typeof_rec = le16_to_cpu(pfi->type);
...
#ifdef DEBUG_USB_LITE
				dev_info(&usb_if->usb_intf->dev,
					"type=%u sizeof=%u can_bus_count=%u\n",
					typeof_rec, sizeof_rec, pfi->can_bus_count);
#endif
#ifdef DEBUG_GUID
				if (sizeof_rec >= PCAN_USBFD_VENDOR_INFO_SIZE_V5) {
					char tmp[4+1+3*PCAN_GUID_LEN+1];
					int l = sprintf(tmp, "GUID=");

					for (i = 0; i < sizeof(pfi->guid); i++)
						l += sprintf(tmp+l, "%02X", pfi->guid[i]);
					sprintf(tmp+l, "\n");

					dev_info(&usb_if->usb_intf->dev, tmp);
				}
#endif
			}

			/* in case of several usb-if are handled by the same
			 * adapter (PCAN-USB X6 for ex)
			 */
			if ((sizeof_rec >= PCAN_USBFD_VENDOR_INFO_SIZE_V5)  &&
			    !(usb_if->adapter->flags & PCAN_ADAPTER_GUID)) {

				usb_if->adapter->flags |= PCAN_ADAPTER_GUID;
				memcpy(usb_if->adapter->guid, pfi->guid,
		...
		usb_if->mcu = pfi->hw_type;

		/* copy the EP numbers if they are provided too
		 *
		 * Note: PCAN-USB FD ATSAM FW 3.5.1: type=5
		 */
		if (typeof_rec >= PCAN_USBFD_TYPE_EXT) {
		...
		for (i = 0; i < usb_if->can_count; i++) {
			...
			/* extended info: get write EP number from struct
			 * Note: PCAN-USB FD ATSAM FW 3.5.1: type=5
			 */
			if (typeof_rec >= PCAN_USBFD_TYPE_EXT)
				  ...
				  const u8 *dev_data)
{
	...
	cmd = canfd_add_cmd(canfd_init_cmd(dev), CANFD_USB_CMD_DEVDATA_SET);
	if (cmd) {
		u16 id = dev_f >> 16;
...
static int pcan_usbfd_get_devdata(struct pcandev *dev, int dev_f, u8 *dev_data)
{
	...
	cmd_req = canfd_add_cmd(canfd_init_cmd(dev), CANFD_USB_CMD_DEVDATA_REQ);
	if (!cmd_req)
		return -ENOMEM;
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(pcan%u, 0x%x)\n",
		__func__, dev->nMinor, clk_mode);
#endif
	cmd = canfd_add_cmd(canfd_init_cmd(dev), CANFD_USB_CMD_CLK_SET);
	if (cmd)
...
{
	struct pcan_usb_interface *usb_if = pcan_usb_get_if(dev);
	struct ucan_usb_io_ctrl *cmd = canfd_add_cmd(canfd_init_cmd(dev), op_req);
				...
				u16 op_req)
{
	...
	cmd = canfd_add_cmd(canfd_init_cmd(dev), op_req);
	if (!cmd)
		return -ENOMEM;
...
int pcan_usb_chip_get_dig_cfg(struct pcandev *dev,
			      struct pcanfd_option *opt, void *c)
{
	return pcan_usb_chip_get_io(dev, opt, c, CANFD_USB_CMD_DPIN_CFG_REQ);
}

int pcan_usb_chip_set_dig_cfg(struct pcandev *dev,
			      struct pcanfd_option *opt, void *c)
{
	return pcan_usb_chip_set_io(dev, opt, c, CANFD_USB_CMD_DPIN_CFG_SET);
}

int pcan_usb_chip_get_dig_val(struct pcandev *dev,
			      struct pcanfd_option *opt, void *c)
{
	return pcan_usb_chip_get_io(dev, opt, c, CANFD_USB_CMD_DPIN_VAL_REQ);
}

int pcan_usb_chip_set_dig_val(struct pcandev *dev,
			      struct pcanfd_option *opt, void *c)
{
	return pcan_usb_chip_set_io(dev, opt, c, CANFD_USB_CMD_DPIN_VAL_SET);
}

int pcan_usb_chip_set_dig_bit(struct pcandev *dev,
			      struct pcanfd_option *opt, void *c)
{
	return pcan_usb_chip_set_io(dev, opt, c, CANFD_USB_CMD_DPIN_SET_HIGH);
}

int pcan_usb_chip_clr_dig_bit(struct pcandev *dev,
			      struct pcanfd_option *opt, void *c)
{
	return pcan_usb_chip_set_io(dev, opt, c, CANFD_USB_CMD_DPIN_SET_LOW);
}

int pcan_usb_chip_get_ana_val(struct pcandev *dev,
			      struct pcanfd_option *opt, void *c)
{
	return pcan_usb_chip_get_io(dev, opt, c, CANFD_USB_CMD_ANAL_VAL_REQ);
}
				     ...
				     struct pcanxl_init *pxli)
{
	...
	case 20000000:
		ucan_usb_set_clck_domain(dev, CANFD_USB_CLK_20MHZ);
		break;
	case 24000000:
		ucan_usb_set_clck_domain(dev, CANFD_USB_CLK_24MHZ);
		break;
	case 30000000:
		ucan_usb_set_clck_domain(dev, CANFD_USB_CLK_30MHZ);
		break;
	case 40000000:
		ucan_usb_set_clck_domain(dev, CANFD_USB_CLK_40MHZ);
		break;
	case 60000000:
		ucan_usb_set_clck_domain(dev, CANFD_USB_CLK_60MHZ);
		break;
	case 80000000:
		ucan_usb_set_clck_domain(dev, CANFD_USB_CLK_80MHZ);
		break;
...
static int pcan_usbfd_open_fd(struct pcandev *dev, struct pcanfd_init *pfdi)
{
	...
	pr_info(DEVICE_NAME ": %s(pcan%u, clk=%u Hz)\n",
		__func__, dev->nMinor, pfdi->clock_Hz);
#endif
	...
	return canfd_device_open_fd(dev, pfdi, usb_opt_to_set, usb_opt_to_clr);
}
...
static int pcan_usbfd_close(struct pcandev *dev)
{
	...
	pr_info(DEVICE_NAME ": %s(pcan%u): opened_count=%d\n",
		__func__, dev->nMinor, usb_if->opened_count);
#endif
				   ...
				   void *rx_msg, void *arg)
{
	...
	return pcan_usbpro_return(dev,
			canfd_post_rxmsg(dev,
					 (struct canfd_rx_msg *)rx_msg,
				   ...
				   void *rx_msg, void *arg)
{
	...
	return pcan_usbpro_return(dev,
			canfd_post_error(dev,
					 (struct canfd_error_msg *)rx_msg,
					 ucan->ops.ts_decoder(dev,
							      rx_msg,
							      &tv)));
}
				      ...
				      void *rx_msg, void *arg)
{
	...
	return pcan_usbpro_return(dev,
			canfd_post_busload(dev,
					   (struct canfd_bus_load_msg *)rx_msg,
				    ...
				    void *rx_msg, void *arg)
{
	...
	int err = canfd_post_status(dev, (struct canfd_status_msg *)rx_msg,
...
#if defined(DEBUG_TRACE) && !defined(DEBUG_USB_LITE)
				 ...
				 u8 *msg_addr, int msg_len)
{
	...
	for (d = 0; d < usb_if->can_count; d++) {
		...
		dev->port.usb.state &= ~PCAN_USB_SHOULD_WAKEUP;
	}
	...
	for (d = 0; d < usb_if->can_count; d++) {
		...
		if (dev->port.usb.state & PCAN_USB_SHOULD_WAKEUP) {
			dev->total_stats.rx_irq_counter++;
			dev->session_stats.rx_irq_counter++;
#ifndef NETDEV_SUPPORT
#ifdef DEBUG_USB
			pr_info(DEVICE_NAME
...
{
	int len = pcan_usbpro_request(usb_if,
...
static void pcan_usbfd_cleanup(struct pcandev *dev)
{
	...
	/* Switch LED off (PCAN-USBXL needs it, even if
	 * pcan_usbpro_driver_loaded() is used next
	 */
	err = send_cmd_set_can_led(dev, CANFD_USB_LED_OFF);
	if (err)
		goto lbl_end;
...
lbl_end:
	...
	if (dev->can_idx == (usb_if->can_count-1)) {
		...
		if (!err)
			send_cmd_clr_dis_option(dev, 0,
						CANFD_USB_OPTION_CALIBRATION);
...
static int pcan_usbfd_ctrl_init(struct pcandev *dev)
{
	...
	err = send_cmd_set_can_led(dev, CANFD_USB_LED_DEF);
...
/*
 * void pcan_usbfd_init_callbacks(struct pcan_usb_interface *usb_if)
 */
void pcan_usbfd_init_callbacks(struct pcan_usb_interface *usb_if)
{
	...
	usb_if->device_ctrl_set_bus_on = canfd_set_bus_on;
	usb_if->device_ctrl_set_bus_off = canfd_set_bus_off;
	...
	/* All devices except PCAN-USB [Pro] FD BL < 2.0.0 */
	usb_if->device_set_mass_storage_mode = pcan_usbfd_set_mass_storage_mode;
}
...
static int pcan_usbfd_identify(struct pcandev *dev, u32 delay_ms)
{
	...
	if (delay_ms) {
		...
		err = send_cmd_set_can_led(dev, CANFD_USB_LED_FAST);
		if (err)
			return err;
	...
	return !err ? CANFD_USB_LED_DEF : err;
}

/* interface functions used to send commands / handle msgs to USB/uCAN */
static int (*pcan_usbfd_msg_handlers[])(struct ucan_engine *,
					void *, void *) = {
	[CANFD_MSG_CAN_RX] = pcan_usbfd_decode_canrx,

	/* use this one if rx/tx errors counters are pushed into rx queue
	 * as STATUS messages
	 */
	[CANFD_MSG_ERROR] = pcan_usbfd_decode_error,
	[CANFD_MSG_STATUS] = pcan_usbfd_decode_status,
	[CANFD_MSG_BUSLOAD] = pcan_usbfd_decode_bus_load,

	[CANFD_USB_MSG_CALIBRATION] = pcan_usbfd_decode_calibration,
	[CANFD_USB_MSG_OVERRUN] = pcan_usbfd_decode_overrun,
};

static struct ucan_ops pcan_usbfd_ucan_ops = {
	.set_clk_domain = pcan_usbfd_set_clk_domain,
	.send_cmd = pcan_usbfd_send_ucan_cmd,
	.tx_msg_encoder = canfd_encode_txmsg,
	.rx_msg_handler = canfd_handle_rxmsg,
	.ts_decoder = pcan_usbfd_ts_decoder,
	.handle_msg_table = pcan_usbfd_msg_handlers,
	.handle_msg_size = ARRAY_SIZE(pcan_usbfd_msg_handlers),
};

struct ucan_ops *pcan_usbfd_init_callbacks_dev(struct pcandev *dev)
{
...
int pcan_usbfd_init(struct pcan_usb_interface *usb_if)
{
	...
	if (VER_NUM(usb_if->hw_ver.major,
		...
		usb_if->device_set_devdata = NULL;
		usb_if->device_get_devdata = NULL;
	}
	...
	case PCAN_USBPROFD_PRODUCT_ID:
		if (bl_version < VER_NUM(2, 0, 0))
			usb_if->device_set_mass_storage_mode = NULL;
		break;
	}
