...
static int pcan_usb_set_bus_on(struct pcandev *dev)
{
	struct pcan_usb_interface *usb_if = pcan_usb_get_if(dev);
	int err;
	...
	if (VER_NUM(usb_if->hw_ver.major,
		    usb_if->hw_ver.minor,
		    usb_if->hw_ver.subminor) <= VER_NUM(2, 8, 0)) {
...
static int pcan_usb_set_bus_off(struct pcandev *dev)
{
	int err;
				...
				u8 *ucMsgPtr, int lCurrentLength)
{
	...
	for (i = 0; i < ucMsgLen; i++) {
		ULCONV localID;
		struct pcanxl_rxmsg_cc rx = { .msg = { .data_len = 0, } };
		struct pcanxl_rxmsg *rxp = (struct pcanxl_rxmsg *)&rx;
		...
		rx.msg.flags = pcanxl_msg_flags_dlc_set(PCANFD_MSG_STD, ucLen);
		...
		/* Status message */
			...
			case 1:
				if ((ucNumber & CAN_RECEIVE_OVERRUN) ||
					...
					pcan_handle_error_ctrl(dev, rxp,
							PCANFD_RX_OVERFLOW);
				}
			...
			case 4: /* only timestamp */
				...
				if (dev->bus_state == PCANFD_UNKNOWN)
					pcan_handle_error_active(dev, rxp);
#endif
				break;
			...
			case 5:
				...
				bus_err.type = (ecc & 0xc0) >> 6;
				bus_err.code = ecc & 0x1f;
				bus_err.rx = ecc & 0x20;
				bus_err.gen = 0;
				...
				pcan_handle_error_msg(dev, rxp, &bus_err);
				break;
	...
	*ptr++ = PCAN_USB_FORMAT_CODE;
	...
	while ((ptr - ucMsgPtr) < nBufferTop) {
		struct pcanxl_txmsg_cc tx = {
			.msg = { .data_len = PCANFD_CAN20_MAXDATALEN, }
		};
		int nRtrFrame;
		u8 dlc;
	...
	if ((ptr - ucMsgPtr) > 2) {
		...
		*ptr = (u8)(dev->port.usb.tx_buffers & 0xff);

		/* last to do: put count of messages */
		*pucMsgCountPtr = nMsgCounter;

		dev->total_stats.tx.frames += nMsgCounter;
		dev->session_stats.tx.frames += nMsgCounter;
...
int pcan_usb_init(struct pcan_usb_interface *usb_if)
{
	...
	usb_if->device_ctrl_set_bus_on = pcan_usb_set_bus_on;
	usb_if->device_ctrl_set_bus_off = pcan_usb_set_bus_off;
