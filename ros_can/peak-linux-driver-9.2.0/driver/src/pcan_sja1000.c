...
{
	u32 dwStart = pcan_get_now_ms();
	u8 tmp;

	tmp = dev->readreg(dev, SJA1000_MOD);
	while (!(tmp & RESET_MODE) &&
			((pcan_get_now_ms() - dwStart) < MODE_REGISTER_SWITCH_TIME)) {
...
static int __sja1000_read(struct pcandev *dev)
{
	...
	for (msgs = 0; msgs < irqmaxrmsg; msgs++) {
		...
		if (fi & BUFFER_EFF) {
			...
			f.msg.flags = pcanxl_msg_flags_dlc_set(MSGTYPE_EXTENDED,
							       dlc);
			...
			f.msg.flags = pcanxl_msg_flags_dlc_set(MSGTYPE_STANDARD,
							       dlc);
...
static int sja1000_write_msg(struct pcandev *dev, struct pcanxl_msg *pf)
{
	ULCONV localID;
	u8 fi, dreg, cmd;
	int i;
	...
	if (fi == PCANFD_CAN20_MAXDATALEN) {
		u8 tmp_dlc = pcanxl_msg_flags_dlc_get(pf->flags);
		if (tmp_dlc > fi)
			fi = tmp_dlc;
	}
	...
	dev->total_stats.tx.bytes += pf->data_len;
	dev->session_stats.tx.bytes += pf->data_len;
...
static int __sja1000_write(struct pcandev *dev, struct pcanusr *ctx)
{
	...
	int err = pcan_txfifo_out(dev, (struct pcanxl_txmsg *)&tx, tx.msg.data);
	if (err >= 0)
		err = sja1000_write_msg(dev, (struct pcanxl_msg *)&tx.msg);
...
irqreturn_t __pcan_sja1000_irqhandler(struct pcandev *dev)
{
	...
	u32 rwakeup = 0;
	u32 wwakeup = 0;
	struct pcanxl_rxmsg ef = { .msg = { .data_len = 0, } };
	...
	for (j = 0; j < irqmaxloop; j++) {
		...
		if (irqstatus & (ERROR_PASSIV_INTERRUPT|ERROR_WARN_INTERRUPT)) {
			...
			pcan_handle_error_msg(dev, &ef, &err);
#ifdef DEBUG
