				...
				u32 sysclock)
{
...
#ifndef CONFIG_64BIT
			/* this only division needs 64-bits do_div().
	...
	if (best_baudrate.penalty == PENALTY_INVALID) {
		pr_err(DEVICE_NAME ": %s(): no baudrate found\n", __func__);
		return 0;
	}
...
#ifdef DEBUG_TIMING
	pr_info(DEVICE_NAME ": %s() best_baudrate[penalty=%u] abt_result["
				...
				u32 sysclock_Hz)
{
...
#ifdef DEBUG_TIMING
		pr_info(DEVICE_NAME ": %s(sysclock=%u): tSCL_ns=%u "
			"pbt[sjw=%u tseg1=%u tseg2=%u brp=%u tsam=%u]\n",
			__func__, sysclock_Hz, tSCL_ns,
	...
	pr_err(DEVICE_NAME ": %s() tSCL_ns=%uGHz/%u=0!!!\n",
		__func__, abt->prescaler, sysclock_Hz);
 ...
 */
static int
pcan_update_sample_point(const struct pcanfd_bittiming_range *btc,
 ...
 *      drivers/net/can/calc_bittiming.c)
 */
int pcan_bitrate_to_bittiming_ex(struct pcan_bittiming *bt,
				 const struct pcanfd_bittiming_range *btc,
				 u32 sysclock_Hz, int pwm_encoding,
				 int desired_brp)
{
	u32 clk_freq;
	...
	if (desired_brp > 0) {
		if ((desired_brp < btc->brp_min) ||
		    (desired_brp > btc->brp_max))
			return -EINVAL;
	...
	if (bt->sample_point) {
		...
		sample_point_ref = bt->sample_point;

	} else if (pwm_encoding) {

		if (bt->bitrate > 15000000)
			/* 62.5% */
			sample_point_ref = 625 * PCAN_SAMPT_SCALE/1000;
		else if (bt->bitrate > 9000000)
			/* 60% */
			sample_point_ref = 600 * PCAN_SAMPT_SCALE/1000;
		else if (bt->bitrate > 4000000)
			/* 56% */
			sample_point_ref = 560 * PCAN_SAMPT_SCALE/1000;
		else
			/* 52% */
			sample_point_ref = 520 * PCAN_SAMPT_SCALE/1000;

	} else {
		/* NRZ default */
		if (bt->bitrate > 800000)
			/* 75% */
			sample_point_ref = 750 * PCAN_SAMPT_SCALE/1000;
		else if (bt->bitrate > 500000)
			/* 80% */
			sample_point_ref = 800 * PCAN_SAMPT_SCALE/1000;
		else
			/* 87,5% */
			sample_point_ref = 875 * PCAN_SAMPT_SCALE/1000;
	}
	...
	for (tseg = (btc->tseg1_max + btc->tseg2_max) * 2 + 1;
		...
		/* CAN-FD/XL have added advice/requirement on BRP value  */
		if (desired_brp <= 0) {

			/* Compute all possible tseg choices
			 * (tseg=tseg1+tseg2)
			 */
		...
		pcan_update_sample_point(btc, sample_point_ref, tseg / 2,
...
#ifdef DEBUG_TIMING
		pr_info(DEVICE_NAME
			": - best[brp=%u tseg=%u btr_err=%d sp_err=%d]\n",
			best_brp, best_tseg, best_bitrate_error,
	...
	/* real sample point */
	bt->sample_point = pcan_update_sample_point(btc, sample_point_ref,
	...
	if (!bt->sjw || !btc->sjw_max) {
		u32 phase_seg1 = tseg1 - (tseg1 / 2);

		bt->sjw = max(1U, min(phase_seg1, tseg2 / 2));
	} else {
