...
#include "src/pcanfd_core_user.h"
 ...
 * are valid, that is:
 * 1/ tv_nsec must belong to [0..999999]
...
#define DEFAULT_FDBITRATE	2000000		/* default FD data btr = 2M */
#define DEFAULT_XLBITRATE	5000000		/* default XL bitrate = 5M */
...
static u32 pcan_def_fd_bitrate = DEFAULT_FDBITRATE;
static u32 pcan_def_xl_bitrate = DEFAULT_XLBITRATE;
...
#include <linux/device.h>
    ...
    !defined(PCAN_SYSFS_ATTR_NEEDS_CONST)
#define PCAN_SYSFS_ATTR_NEEDS_CONST
#endif
...
/* KMALLOC_MAX_SIZE /		4194304 /	= PCAN_DEV_RXQSIZE_MAX
 * struct pcanfd_rxmsg		152		= 27594 (pcan v8)
 * struct pcanxl_rxmsg		88		= 47662
 * struct pcanxl_rxmsg_fd	152		= 27594
 * struct pcanxl_rxmsg_xl	2136		= 1963
 *
 * Since v9 fifo saves variable length items, keep v8 MAX value for backward
 * compatibility reason only.
 *
 * But, in case of CANXL, MIN value SHOULD evolved:
 * rxqsize=2 means 2 * 2136 = 4272 / 152 ~ 29 slots.
 */
#define PCAN_DEV_RXQSIZE_MIN_V8	2
#define PCAN_DEV_RXQSIZE_MIN	29
#define PCAN_DEV_RXQSIZE_MAX	(KMALLOC_MAX_SIZE / \
				 sizeof(struct pcanxl_rxmsg_fd))
...
#define PCANFD_BERRLOG_LEVEL_NONE	0
#define PCANFD_BERRLOG_LEVEL_LIMITED	1
#define PCANFD_BERRLOG_LEVEL_FULL	2

static ushort berrlog = PCANFD_BERRLOG_LEVEL_NONE;
module_param(berrlog, ushort, 0644);
MODULE_PARM_DESC(berrlog,
		 " bus error logging level (0=none, 1=limited, 2=full)");

/* KMALLOC_MAX_SIZE /		4194304 /	= PCAN_DEV_RXQSIZE_MAX
 * struct pcanfd_txmsg		136		= 30840 (pcan v8)
 * struct pcanxl_txmsg		72		= 58254
 * struct pcanxl_txmsg_fd	136		= 30840
 * struct pcanxl_txmsg_xl	2120		= 1978
 *
 * Since v9 fifo saves variable length items, keep v8 MAX value for backward
 * compatibility reason only.
 *
 * But, in case of CANXL, MIN value SHOULD evolved:
 * txqsize=2 means 2 * 2120 = 4240 / 136 ~ 32 slots.
 */
#define PCAN_DEV_TXQSIZE_MIN_V8	2
#define PCAN_DEV_TXQSIZE_MIN	32
#define PCAN_DEV_TXQSIZE_MAX	(KMALLOC_MAX_SIZE / \
				 sizeof(struct pcanxl_txmsg_fd))
...
/*
 * int __pcan_set_dev_opt(struct pcandev *pdev, int o, u32 v)
 *
 * Since options are set from user space, this function bypasses copy_user()
 * so that any VALID option can be set from kernel space too.
 *
 * WARNING: @o MUST be valid, no test is done on the value!
 */
int __pcan_set_dev_opt(struct pcandev *pdev, int o, u32 v)
{
	struct pcanfd_option opt = {
		.size = sizeof(v),
		.name = PCANXL_OPT_MAX,	/* trick to avoid copy_to_user() */
		.value = &v,
	};

	if (!pdev->option[o].set) {
		pr_warn(DEVICE_NAME
			": %s(opt=%u): ABNORMAL set() NULL pointer\n",
			__func__, o);
		return -EOPNOTSUPP;
	}

	return pdev->option[o].set(pdev, &opt, NULL);
}

/*
 * int __pcan_get_dev_opt(struct pcandev *pdev, int o, u32 *v)
 *
 * Since options are got from user space, this function bypasses copy_user()
 * so that any VALID option can be read from kernel space too.
 *
 * WARNING: @o MUST be valid, no test is done on the value!
 */
int __pcan_get_dev_opt(struct pcandev *pdev, int o, u32 *v)
{
	...
	if (!pdev->option[o].get) {
		pr_warn(DEVICE_NAME
			": %s(opt=%u): ABNORMAL get() NULL pointer\n",
			__func__, o);
		return -EOPNOTSUPP;
	}

	return pdev->option[o].get(pdev, &opt, NULL);
}
	...
	if (pdev && pdev->adapter) {
		...
		for (i = 0; i < sizeof(pdev->adapter->guid); i++) {
			...
			case 10:
				...
				fallthrough;
			default:
				l += scnprintf(buf+l, PAGE_SIZE, "%02x",
					       pdev->adapter->guid[i]);
			}
		}
	}
					   ...
					   char *buf)
{
	struct pcandev *pdev = to_pcandev(dev);
	u32 v;
	int err = __pcan_get_dev_opt(pdev, PCANFD_OPT_MASS_STORAGE_MODE, &v);
	if (err)
		return err;
					    ...
					    const char *buf, size_t count)
{
	...
	u32 v = simple_strtoul(buf, &endptr, 0);
	if (*endptr != '\n')
		return -EINVAL;

	err = __pcan_set_dev_opt(pdev, PCANFD_OPT_MASS_STORAGE_MODE, v);
	if (err)
		return err;
...
{
	u32 v;
	int err = __pcan_get_dev_opt(to_pcandev(dev), PCANFD_OPT_FLASH_LED, &v);
	if (err)
		return err;
				   ...
				   const char *buf, size_t count)
{
	...
	err = __pcan_set_dev_opt(to_pcandev(dev), PCANFD_OPT_FLASH_LED, v);
	if (err)
		return err;
				  ...
				  const char *buf, size_t count)
{
	...
	err = __pcan_set_ts_mode(pdev, v, false);
	if (err)
		return err;
...
#ifdef NO_RT
	return scnprintf(buf, PAGE_SIZE, "/dev/pcan%s%u\n",
			pdev->type, pdev->nMinor);
#else
	return scnprintf(buf, PAGE_SIZE, "pcan%u\n",
			pdev->nMinor);
#endif
}
...
{
	struct pcandev *pdev = to_pcandev(dev);
	return scnprintf(buf, PAGE_SIZE, "%u.%02u\n",
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (pdev && pdev->adapter)
		l += scnprintf(buf+l, PAGE_SIZE, "%s", pdev->adapter->name);
	else
		l += scnprintf(buf+l, PAGE_SIZE, "pdev=%p adapter=%p",
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (pdev->adapter && pdev->adapter->part_num)
		l += scnprintf(buf+l, PAGE_SIZE, "%s", pdev->adapter->part_num);
				...
				struct device_attribute *attr, char *buf)
{
	...
	if (pdev->hw_ver && pdev->hw_ver->major >= 0) {
		l += scnprintf(buf+l, PAGE_SIZE, "%u", pdev->hw_ver->major);
		if (pdev->hw_ver->minor >= 0) {
			l += scnprintf(buf+l, PAGE_SIZE, ".%u",
				       pdev->hw_ver->minor);
...
{
	struct pcandev *pdev = to_pcandev(dev);
	u32 fifo_ratio = kfifo_ratio(&pdev->rx_fifo.kfifo);
	return scnprintf(buf, PAGE_SIZE, "%u.%02u\n",
...
{
	struct pcandev *pdev = to_pcandev(dev);
	u32 fifo_ratio = kfifo_ratio(&pdev->tx_fifo.kfifo);
	return scnprintf(buf, PAGE_SIZE, "%u.%02u\n",
			 fifo_ratio / 100, fifo_ratio % 100);
}
...
#else
	u32 dev_read = pdev->total_stats.rx.frames;
#endif
...
#else
	u32 dev_write = pdev->total_stats.tx.frames;
#endif
	return show_u32(buf, dev_write);
}

static ssize_t show_pcan_rx_frames(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->total_stats.rx.frames);
}

static ssize_t show_pcan_tx_frames(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->total_stats.tx.frames);
}
...
{
	struct pcandev *pdev = to_pcandev(dev);
	return show_u32(buf, pdev->total_stats.rx_irq_counter +
			     pdev->total_stats.tx_irq_counter);
}

static ssize_t show_pcan_rx_irqs(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->total_stats.rx_irq_counter);
}

static ssize_t show_pcan_tx_irqs(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->total_stats.tx_irq_counter);
}

static ssize_t show_pcan_errors(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->total_stats.error_counter);
}
...
#else
static PCAN_DEVICE_ATTR(sample_point, nom_sp, show_pcan_nsp);
#endif
...
static PCAN_DEVICE_ATTR(rx_frames, rx_frames, show_pcan_rx_frames);
static PCAN_DEVICE_ATTR(tx_frames, tx_frames, show_pcan_tx_frames);
...
static struct attribute *pcan_dev_sysfs_attrs[] = {
	...
	&pcan_dev_attr_rx_frames.attr,
	&pcan_dev_attr_tx_frames.attr,
...
static ssize_t show_pcan_dbitrate(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.fd_data.bitrate);
}
...
{
	return show_u32(buf,
			to_pcandev(dev)->init_settings.fd_data.sample_point);
}

static ssize_t show_pcan_data_tq(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.fd_data.tq);
}

static ssize_t show_pcan_data_brp(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.fd_data.brp);
}

static ssize_t show_pcan_data_tseg1(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.fd_data.tseg1);
}

static ssize_t show_pcan_data_tseg2(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.fd_data.tseg2);
}

static ssize_t show_pcan_data_sjw(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.fd_data.sjw);
}
...
#else
static PCAN_DEVICE_ATTR(dbitrate, fd_bitrate, show_pcan_dbitrate);
static PCAN_DEVICE_ATTR(dsample_point, fd_sp, show_pcan_dsp);
static PCAN_DEVICE_ATTR(data_tq, fd_tq, show_pcan_data_tq);
static PCAN_DEVICE_ATTR(data_brp, fd_brp, show_pcan_data_brp);
static PCAN_DEVICE_ATTR(data_tseg1, fd_tseg1, show_pcan_data_tseg1);
static PCAN_DEVICE_ATTR(data_tseg2, fd_tseg2, show_pcan_data_tseg2);
static PCAN_DEVICE_ATTR(data_sjw, fd_sjw, show_pcan_data_sjw);
#endif
...
static ssize_t show_pcan_xl_dbitrate(struct device *dev,
				     struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_data.bitrate);
}

static ssize_t show_pcan_xl_sp(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	return show_u32(buf,
			to_pcandev(dev)->init_settings.xl_data.sample_point);
}

static ssize_t show_pcan_xl_tq(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_data.tq);
}

static ssize_t show_pcan_xl_tseg1(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_data.tseg1);
}

static ssize_t show_pcan_xl_tseg2(struct device *dev,
				  struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_data.tseg2);
}

static ssize_t show_pcan_xl_sjw(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_data.sjw);
}

static ssize_t show_pcan_xl_ssp_offset(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_data.ssp_offset);
}

static ssize_t show_pcan_fd_ssp_offset(struct device *dev,
				       struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.fd_data.ssp_offset);
}

static ssize_t show_pcan_xl_es(struct device *dev,
			       struct device_attribute *attr, char *buf)
{
	u32 es = !(to_pcandev(dev)->init_settings.flags & PCANXL_INIT_ES_OFF);
	return show_u32(buf, es);
}

static ssize_t show_pcan_xl_tms(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	u32 tms = !!(to_pcandev(dev)->init_settings.flags & PCANXL_INIT_TMS_ON);
	return show_u32(buf, tms);
}

static ssize_t show_pcan_xl_pwms(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_pwm.pwm_short);
}

static ssize_t show_pcan_xl_pwml(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_pwm.pwm_long);
}

static ssize_t show_pcan_xl_pwmo(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	return show_u32(buf, to_pcandev(dev)->init_settings.xl_pwm.pwm_offset);
}

static PCAN_DEVICE_ATTR(xl_bitrate, xl_bitrate, show_pcan_xl_dbitrate);
static PCAN_DEVICE_ATTR(xl_sp, xl_sp, show_pcan_xl_sp);
static PCAN_DEVICE_ATTR(xl_tq, xl_tq, show_pcan_xl_tq);
static PCAN_DEVICE_ATTR(xl_tseg1, xl_tseg1, show_pcan_xl_tseg1);
static PCAN_DEVICE_ATTR(xl_tseg2, xl_tseg2, show_pcan_xl_tseg2);
static PCAN_DEVICE_ATTR(xl_sjw, xl_sjw, show_pcan_xl_sjw);
static PCAN_DEVICE_ATTR(xl_ssp_offset, xl_ssp_offset, show_pcan_xl_ssp_offset);
static PCAN_DEVICE_ATTR(fd_ssp_offset, fd_ssp_offset, show_pcan_fd_ssp_offset);
static PCAN_DEVICE_ATTR(xl_es, xl_es, show_pcan_xl_es);
static PCAN_DEVICE_ATTR(xl_tms, xl_tms, show_pcan_xl_tms);
static PCAN_DEVICE_ATTR(xl_pwms, xl_pwms, show_pcan_xl_pwms);
static PCAN_DEVICE_ATTR(xl_pwml, xl_pwml, show_pcan_xl_pwml);
static PCAN_DEVICE_ATTR(xl_pwmo, xl_pwmo, show_pcan_xl_pwmo);

static struct attribute *pcan_dev_sysfs_xl_attrs[] = {
	&pcan_dev_attr_xl_bitrate.attr,
	&pcan_dev_attr_xl_sp.attr,
	&pcan_dev_attr_xl_tq.attr,
	&pcan_dev_attr_xl_tseg1.attr,
	&pcan_dev_attr_xl_tseg2.attr,
	&pcan_dev_attr_xl_sjw.attr,
	&pcan_dev_attr_xl_ssp_offset.attr,
	&pcan_dev_attr_fd_ssp_offset.attr,
	&pcan_dev_attr_xl_es.attr,
	&pcan_dev_attr_xl_tms.attr,
	&pcan_dev_attr_xl_pwms.attr,
	&pcan_dev_attr_xl_pwml.attr,
	&pcan_dev_attr_xl_pwmo.attr,

	NULL
};

static PCAN_DEVICE_ATTR(bus_load, bus_load, show_pcan_bus_load);
static PCAN_DEVICE_ATTR(rx_err_cnt, rx_errors, show_pcan_rx_err_cnt);
static PCAN_DEVICE_ATTR(tx_err_cnt, tx_errors, show_pcan_tx_err_cnt);
...
void pcan_sysfs_dev_node_create_ex(struct pcandev *dev, struct device *parent)
{
	...
	if (!IS_ERR(dev->sysfs_dev)) {
		...
		if (dev->features & PCAN_DEV_XL_RDY)
			pcan_sysfs_add_attrs(dev->sysfs_dev,
						pcan_dev_sysfs_xl_attrs);
					...
					struct pcanxl_rxmsg *pqm)
{
	...
	int fix_ts = 0;
	struct pcanxl_msg *pm = (struct pcanxl_msg *)&pqm->msg;
	int s = 1;
	u64 dns;
	signed long long dtv;
	...
	case PCANFD_OPT_HWTIMESTAMP_RAW:
		 ...
		 */
		pm->timestamp.tv_nsec = do_div(pqm->hwtv.hw_ns, NSEC_PER_SEC);
		pm->timestamp.tv_sec = (__kernel_time_t )pqm->hwtv.hw_ns;
...
#endif
			pr_info(DEVICE_NAME
				": RAW pcan%d %llu %lld.%09lld\n",
				dev->nMinor, pqm->hwtv.hw_ns,
				pm->timestamp.tv_sec, pm->timestamp.tv_nsec);
#endif
		return pm;
	...
	pm->timestamp = pqm->hwtv.host_base_ns;
	 ...
	 *
	 * hw_ns      = hw timestamp of the event
	 * hw_base_ns = hw timestamp of last sync
	 * => dns = count of hw ns since last sync
	 *
	 ...
	 */
	if (unlikely(pqm->hwtv.hw_ns < pqm->hwtv.hw_base_ns)) {
		dns = pqm->hwtv.hw_base_ns - pqm->hwtv.hw_ns;
		s = -1;
	} else {
		dns = pqm->hwtv.hw_ns - pqm->hwtv.hw_base_ns;
	}

	/* Note: dns == 0 means that this event is the first resync'ed */
	if (!dns) {
	...
	//} else if (pqm->hwtv.ts_mode == PCANFD_OPT_HWTIMESTAMP_COOKED &&
		...
		dns <<= PCAN_CLOCK_DRIFT_SCALE_SHIFT;
		 ...
		 */
		do_div(dns, pqm->hwtv.clock_drift);
#endif
	}
	 ...
	 */
	dtv = s * dns - dev->time_sync.evt_time_lag;

	/* timeval_add_us() returns valid tv_sec and tv_usec */
	pcan_timespec_add_ns(&pm->timestamp, dtv);
...
#endif
		pr_info(DEVICE_NAME
			": COOK pcan%d %llu %lld.%09lld\n",
			dev->nMinor, pqm->hwtv.hw_ns,
			pm->timestamp.tv_sec, pm->timestamp.tv_nsec);
#endif

	pcan_gettimeofday_ns(&now);
	dtv = pcan_timespec_diff(&pm->timestamp, &now);
	if (dtv > 0) {
	...
	if (fix_ts) {
		...
		if (fix_ts > 1) {
			...
			pr_err(DEVICE_NAME ": WARNING: %s ts fixed(%d) from: "
			       "%lld.%09lld to: %lld.%09lld (now)\n",
				dev->adapter->name, fix_ts,
				pm->timestamp.tv_sec,
				pm->timestamp.tv_nsec,
				now.tv_sec, now.tv_nsec);

			pr_info(DEVICE_NAME ": ts[base=%llu off=%llu cd=%lu]\n",
				pqm->hwtv.hw_base_ns, pqm->hwtv.hw_ns,
				pqm->hwtv.clock_drift);
#endif
			pm->flags &= ~PCANFD_HWTIMESTAMP;
		        ...
		        struct pcan_timespec *hwtv)
{
...
#endif
		pr_info(DEVICE_NAME
			": DEC pcan%d %llu %llu %u\n",
			dev->nMinor, hwtv->hw_ns, dev->time_sync.evt_base_ns,
			hwtv->ts_mode);
#endif
	...
	default:
...
#endif
		hwtv->hw_base_ns = dev->time_sync.evt_base_ns;
		...
		if (!hwtv->hw_base_ns) {
			...
			pcan_gettimeofday_ns(&dev->time_sync.evt_base_tv);
		}

		/* otherwise, save all the necessary stuff for user to compute
		 * host (cooked) timestamp later...
		 */
		hwtv->host_base_ns = dev->time_sync.evt_base_tv;
	...
	case PCANFD_OPT_HWTIMESTAMP_RAW:
		 ...
		 */
		pcan_gettimeofday_ns(&hwtv->host_base_ns);
		return 1;
...
{
	int err = 0;
	signed long long dts_ns = 0, dtv_ns = 0;
	struct pcan_time_sync now = {
		.ts_ns = ts_ns,
		.clock_drift = dev->time_sync.clock_drift,

		.evt_base_ns = dev->time_sync.evt_base_ns,
	...
	pcan_gettimeofday_ns(&now.tv);
	tv_off *= NSEC_PER_USEC;

	if (!dev->time_sync.ts_ns) {

		/* get host time between substract any host time offset */
		pcan_timespec_add_ns(&now.tv, tv_off);
	...
	dtv_ns = pcan_timespec_to_ns(&now.tv) - pcan_timespec_to_ns(&dev->time_sync.tv);
	if (dtv_ns <= 0) {
	...
	if (dts_ns < 0) {
		 ...
		 */
		if (now.ts_ns)
			pr_warn(DEVICE_NAME "%d: abnormal sync times: "
				"ts=%llu < ts=%llu\n",
				dev->nMinor, now.ts_ns, dev->time_sync.ts_ns);
#endif
		err = 2;
		goto lbl_exit;
	}
	...
	if (dtv_ns <= 0) {
		...
		if (dtv_ns < 0)
			pr_warn(DEVICE_NAME "%d: abnormal sync times: "
				"tv=%lld.%09lld < tv=%lld.%09lld dtv_ns=%lld "
				"tv_off=%d. Did host time change?\n",
				dev->nMinor,
				(long long)now.tv.tv_sec,
				(long long)now.tv.tv_nsec,
				(long long)dev->time_sync.tv.tv_sec,
				(long long)dev->time_sync.tv.tv_nsec,
				dtv_ns, tv_off);
	 ...
	 * periodically:
	 */
	if (dev->time_sync.ttv_ns > 120ULL*NSEC_PER_SEC) {
		dev->time_sync.ttv_ns = 0;
		dev->time_sync.tts_ns = 0;
	}

	/* Now, calculate clock drift by comparing count of ns in both
	 * times:
	 */

	/* ttv_ns = count of host ns since start of sync */
	now.ttv_ns = dev->time_sync.ttv_ns + dtv_ns;

	/* tts_ns = count of hw ns since start of sync */
	now.tts_ns = dev->time_sync.tts_ns + dts_ns;

	/* prevent from div by 0 */
	if (now.ttv_ns) {

		/* this test is useless if ttv_ns and tts_ns are periodically
		 * reset to 0
		 */
		u64 d = now.tts_ns << PCAN_CLOCK_DRIFT_SCALE_SHIFT;
		int diff;
		 ...
		 */
		now.clock_drift = DIV_ROUND_UP_SECTOR_T(d, now.ttv_ns);
...
#ifdef DEBUG_TS
			pr_info(DEVICE_NAME
				"%d: ABNORMAL clock drift value %ld "
...
lbl_exit:
	...
	{
		if (!err)
			pr_info(DEVICE_NAME "%d: SYNC "
				"%llu %lld.%09lld %llu %llu %ld\n",
				dev->nMinor,
				dev->time_sync.ts_ns,
				dev->time_sync.tv.tv_sec,
				dev->time_sync.tv.tv_nsec,
				dtv_ns, dts_ns,
				dev->time_sync.clock_drift);
		else
			pr_info(DEVICE_NAME "%d: SYNC err=%d\n",
				dev->nMinor, err);
	}
#endif
 ...
 * bus state
 */
static int pcan_fix_msg_errcnt(struct pcandev *dev, struct pcanxl_rxmsg *rx)
{
...
static void pcan_push_bus_load_ind(struct timer_list *t)
{
	...
	pr_info(DEVICE_NAME "%d: bus_state=%d bl_count=%d bl_total=%d "
		"bl=%d prev_bl=%d\n", dev->nMinor, dev->bus_state,
		dev->bus_load_count, dev->bus_load_total, dev->bus_load,
		dev->posted.bus_load);
#endif
	...
	if (dev->bus_load_count) {
		dev->bus_load = dev->bus_load_total / dev->bus_load_count;
		dev->bus_load_total = 0;
		dev->bus_load_count = 0;
	} else {
		/* WTF? no BUS_LOAD message received from the HW during the
		 * period?
		 * For PCAN-USB XL, this means "bus load = 100%" (empirical
		 * method...)
		 */
		if (dev->wType == HW_USB_XL)
			dev->bus_load = 10000;
	}

	bl_diff = dev->bus_load - dev->posted.bus_load;

	/* post bus load change if it's greater than 0.5%
	 * or if bus load reaches 0 and previous one wasn't
	 */
	if ((bl_diff >= 50) || (bl_diff <= -50) ||
		(!dev->bus_load && dev->posted.bus_load) ||
		(dev->bus_load && !dev->posted.bus_load)) {
		...
		pcan_gettimeofday_ns(&rx.hwtv.host_base_ns);
		 ...
		 */
		if (pcan_rxfifo_in(dev, &rx, NULL) > 0) {
#ifdef DEBUG_BUS_LOAD
			pr_info(DEVICE_NAME "%d: posted bus load=%d.%02d%%\n",
				dev->nMinor,
				rx.msg.ctrlr_data[PCANFD_BUSLOAD_UNIT],
				rx.msg.ctrlr_data[PCANFD_BUSLOAD_DEC]);
#endif
			dev->posted.bus_load = dev->bus_load;
			pcan_event_signal(&dev->in_event);
		} else {
			dev->posted.bus_load = 0;	/* force next post */
		}
	}
...
/* x = (x >= y) ? x - y : 0; */
static int subtract_timespec(struct __kernel_timespec *x,
			     const struct __kernel_timespec *y)
{
	if (x->tv_nsec >= y->tv_nsec) {
		...
		x->tv_sec--;
		x->tv_nsec += (NSEC_PER_SEC - y->tv_nsec);
...
static void to_drv_rel_time(struct __kernel_timespec *tv)
{
	if (!subtract_timespec(tv, &pcan_drv.sInitTime)) {
		...
		pr_info(DEVICE_NAME
			...
			pcan_drv.sInitTime.tv_sec,
			pcan_drv.sInitTime.tv_nsec);
#endif
...
static void to_dev_rel_time(struct pcandev *dev,
			    struct __kernel_timespec *tv)
{
	if (!subtract_timespec(tv, &dev->init_timestamp)) {
		...
		pr_info(DEVICE_NAME
			...
			"dev \"%lld.%09lld s.\"\n",
			tv->tv_sec, tv->tv_nsec,
			dev->init_timestamp.tv_sec,
			dev->init_timestamp.tv_nsec);
#endif
		tv->tv_sec = tv->tv_nsec = 0;
	}
}

static int pcan_status_rx(struct pcandev *dev, struct pcanxl_rxmsg *rx)
{
	int rxerr = dev->posted.rxerr;
	int txerr = dev->posted.txerr;

#if defined(DEBUG_TRACE) || defined(DEBUG_BUS_STATE)
	pr_info(DEVICE_NAME ": %s(pcan%d, rx[id=%u]) "
			"bus=%u rx=%u/%u tx=%u/%u\n", __func__, dev->nMinor,
...
#ifdef DEBUG_BUS_STATE
		pr_info(DEVICE_NAME "%d: msg.id %d != %d: post-it!\n",
			dev->nMinor, rx->msg.id, dev->posted.bus_state);
#endif
		goto lbl_post_it;
	}
...
#ifdef DEBUG_BUS_STATE
			pr_info(DEVICE_NAME "%d: rxerr %d != %d: post-it!\n",
				dev->nMinor, rxerr, dev->posted.rxerr);
#endif
			goto lbl_post_it;
		}
...
#ifdef DEBUG_BUS_STATE
			pr_info(DEVICE_NAME "%d: txerr %d != %d: post-it!\n",
				dev->nMinor, txerr, dev->posted.txerr);
#endif
			goto lbl_post_it;
		}
	}
...
static int pcan_error_rx(struct pcandev *dev, struct pcanxl_rxmsg *rx)
{
	int rxerr = dev->posted.rxerr;
	int txerr = dev->posted.txerr;
...
/*
 * int msgcmp(struct pcanxl_msg *rx1, struct pcanxl_msg *rx2)
 *
 ...
 *	0	message are equal
 */
static int msgcmp(struct pcanxl_msg *msg1, struct pcanxl_msg *msg2)
{
 ...
 */
static void pcan_fix_msg_timestamp(struct pcandev *dev, struct pcanxl_rxmsg *rx)
{
	...
	if (rx->hwtv.ts_mode != PCANFD_OPT_HWTIMESTAMP_RAW) {
		...
		case PCANFD_INIT_TS_DRV_REL:
			to_drv_rel_time(&rx->hwtv.host_base_ns);
			break;

		case PCANFD_INIT_TS_DEV_REL: 
			to_dev_rel_time(dev, &rx->hwtv.host_base_ns);
			break;
...
#ifdef DEBUG
	pr_info(DEVICE_NAME ": %s(): tv=%lld.%09lld s\n", __func__,
		rx->hwtv.host_base_ns.tv_sec, rx->hwtv.host_base_ns.tv_nsec);
#endif
}
...
int __pcan_chardev_rx(struct pcandev *dev, struct pcanxl_rxmsg *rx, u8 *data)
{
	...
	//if (dev->wType == HW_USB_PRO_FD)
		...
		default:
			pr_info(DEVICE_NAME
				": %s(pcan%d > [type=%u id=%u flg=%xh "
				"len=%u])\n",
				__func__, dev->nMinor, rx->msg.type,
				rx->msg.id, rx->msg.flags, rx->msg.data_len);
			break;
		}
#endif
	...
	case PCANXL_TYPE_CANXL:
		...
		/* next is not relevant for CAN-XL */
		if (rx->msg.type == PCANXL_TYPE_CANXL)
			break;
	...
	/* push frame into Rx fifo.
	 * don't return any error to not stop frames processing when they're
	 * received by packets.
	 */
	err = pcan_rxfifo_in(dev, rx, data);
	if (err >= 0) {
#if defined(DEBUG_RX_FIFO) || defined(DEBUG_BUS_STATE)
#ifdef DEBUG_BUS_STATE
		if (rx->msg.type == PCANFD_TYPE_STATUS)
#endif
			pr_info(DEVICE_NAME "%d: Rx FIFO < "
				"type=%u id=%u data_len=%d (err %d)\n",
				dev->nMinor, rx->msg.type, rx->msg.id,
				rx->msg.data_len, err);
#endif
		return err;
	}

	dev->total_stats.rx_lost++;
	dev->session_stats.rx_lost++;

	if (err == -ENOSPC) {

		/* limit dmesg log for the current session */
		if (dev->session_stats.rx_lost <= 3)
			pr_err(DEVICE_NAME "%d: %s CAN%u: "
			       "msg[type=%d id=%xh l=%u] lost: "
			       "err %d rxqsize=%u\n",
			       dev->nMinor, dev->adapter->name, pcan_idx(dev)+1,
			       rx->msg.type, rx->msg.id, rx->msg.data_len,
			       err, rxqsize);

		/* it remains one slot to save one more header in Rx fifo:
		 * put a STATUS[PCANFD_RX_OVERFLOW]
		 */
		pcan_handle_error_internal(dev, rx, PCANFD_RX_OVERFLOW);
		err = kfifo_in(&dev->rx_fifo.kfifo, rx, sizeof(*rx));
		if (err == sizeof(*rx))
			return err;

		pr_err(DEVICE_NAME "%d: unable to save "
		       "PCANFD_RX_OVERFLOW into fifo: av=%d err=%d\n",
		       dev->nMinor, kfifo_avail(&dev->rx_fifo.kfifo), err);
	}

	/* In case of error, SHOULD NOT return any error to not stop frames
	 * processing when they're received by packets.
	 * Returning 0 prevent from signaling any event next, because nothing
	 * has actually been pushed into the fifo.
	 */
	return 0;
}
...
int pcan_post_bus_state(struct pcandev *dev)
{
	...
	if (dev->bus_state != PCANFD_UNKNOWN) {
		...
		pr_err(DEVICE_NAME ": %s(pcan%d): posting UNKNOWN bus_state\n",
		       __func__, dev->nMinor);
#endif
	}
 ...
 *
 * Used by any CAN frame Rx handler to check whether context is ok to post the
 * Rx frame to user.
 */
int pcan_post_rxmsg_is_ok(struct pcandev *dev)
{
	...
	case PCANFD_ERROR_BUSOFF:
#ifdef DEBUG_RX_PATH
		pr_info(DEVICE_NAME "%d: rx CAN msg discarded (bus_state=%u)\n",
...
{
#if defined(DEBUG_TRACE) || defined(DEBUG_BUS_STATE)
	pr_info(DEVICE_NAME ": %s(pcan%d): bus=%u rx=%u/tx=%u\n",
		__func__, dev->nMinor, dev->bus_state,
...
int pcan_set_bus_state(struct pcandev *dev, enum pcanfd_status bus_state)
{
	...
	case PCANFD_ERROR_BUSOFF:
		...
		if (pcan_set_status_bit(dev, CAN_ERR_BUSOFF)) {
			dev->total_stats.error_counter++;
			dev->session_stats.error_counter++;
		}
		break;
	case PCANFD_UNKNOWN:
#ifdef DEBUG_BUS_STATE
	...
	/* if bus state goes to PASSIVE (or BUSOFF) then no need to wait
	 * for room in Tx FIFO
	 */
	if (bus_state >= PCANFD_ERROR_PASSIVE)
		pcan_event_signal(&dev->out_event);

	return 1;
}

int pcan_handle_busoff(struct pcandev *dev, struct pcanxl_rxmsg *rx)
{
	...
	pr_warn(DEVICE_NAME "%d: %s CAN%u %u: BUS OFF\n", dev->nMinor,
		dev->adapter->name, pcan_idx(dev)+1, dev->adapter->index);
...
void pcan_handle_error_active(struct pcandev *dev, struct pcanxl_rxmsg *rx)
{
	const enum pcanfd_status bus_state = dev->bus_state;

#if defined(DEBUG_TRACE) || defined(DEBUG_BUS_STATE)
	pr_info(DEVICE_NAME ": %s(pcan%d): bus=%u rx=%u/tx=%u (@rx=%p)\n",
		__func__, dev->nMinor, bus_state,
	...
	if (rx) {
		...
		if (bus_state != PCANFD_ERROR_ACTIVE) {
			...
			if (bus_state == PCANFD_ERROR_BUSOFF)
				pr_info(DEVICE_NAME "%d: %s CAN%u %u: ACTIVE\n",
 ...
 * Return 1 if not.
 */
int pcan_handle_error_status(struct pcandev *dev, struct pcanxl_rxmsg *rx,
			     int err_warning, int err_passive)
{
#if defined(DEBUG_TRACE) || defined(DEBUG_BUS_STATE)
	pr_info(DEVICE_NAME ": %s(pcan%d, EW=%u, EP=%u): "
...
int pcan_handle_error_ctrl(struct pcandev *dev, struct pcanxl_rxmsg *rx,
			   int err_ctrlr)
{
#if defined(DEBUG_TRACE)
	pr_info(DEVICE_NAME ": %s(pcan%d, err_ctrlr=%d)\n",
		__func__, dev->nMinor, err_ctrlr);
#endif
	dev->total_stats.error_counter++;
	dev->session_stats.error_counter++;
	...
	case PCANFD_RX_OVERFLOW:
		...
		if (printk_ratelimit())
			pr_err(DEVICE_NAME "%d: %s CAN%u %u: "
			       "controller internal Rx queue overflow: "
			       "possible loss of msgs(s)\n",
			       dev->nMinor, dev->adapter->name,
			       pcan_idx(dev)+1, dev->adapter->index);
	...
	case PCANFD_TX_OVERFLOW:
		...
		pr_err(DEVICE_NAME "%d: %s CAN%u %u: "
		       "controller internal Tx queue overflow: "
		       "possible loss of msgs(s)\n",
		       dev->nMinor, dev->adapter->name, pcan_idx(dev)+1,
		       dev->adapter->index);
		...
		break;
	default:
		if (!pcan_set_status_bit(dev, CAN_ERR_RESOURCE))
			return 0;

		pr_err(DEVICE_NAME "%d: %s CAN%u %u: "
		       "controller internal error %d\n",
		       dev->nMinor, dev->adapter->name, pcan_idx(dev)+1,
		       dev->adapter->index, err_ctrlr);
		break;
	}
...
void pcan_handle_error_msg(struct pcandev *dev, struct pcanxl_rxmsg *rx,
			   struct pcan_bus_error *err)
{
	int bus_err_notif = 0;
	static const char *bus_err_txt[PCANFD_ERRMSG_COUNT] = {
		[PCANFD_ERRMSG_BIT] = "BIT_ERR",
		[PCANFD_ERRMSG_FORM] = "FRM_ERR",
		[PCANFD_ERRMSG_STUFF] = "STF_ERR",
		[PCANFD_ERRMSG_OTHER] = "OTH_ERR",
	};

#if defined(DEBUG_TRACE)
	pr_info(DEVICE_NAME ": %s(pcan%d, bus_err[type=%d code=%d])\n",
		__func__, dev->nMinor, err->type, err->code);
#endif

	if (rx) {
		rx->msg.type = PCANXL_TYPE_ERROR;
		rx->msg.id = err->type;
		if (err->rx)
		...
		/* new pcan v9:
		 * PCANXL_TYPE_ERROR saves err_code into ctrlr_data[]
		 * while PCANFD_TYPE_ERROR_MSG saved it into data[].
		 */
		rx->msg.ctrlr_data[PCANXL_ERRCODE] = err->code;
		rx->msg.data_len = 0;

		bus_err_notif = (rx->msg.flags & PCANXL_ERROR_SIGN_OFF);
	}
	...
	switch (berrlog) {
	case PCANFD_BERRLOG_LEVEL_LIMITED:
		if (!printk_ratelimit())
			break;
		
		/* fallthrough */
		fallthrough;
	case PCANFD_BERRLOG_LEVEL_FULL:
		pr_warn(DEVICE_NAME
			"%u: %s CAN%u %u: BUS_ERROR%s=%s "
			"dir=%s code=%d (gen=%c)\n",
			dev->nMinor, dev->adapter->name, pcan_idx(dev)+1,
			dev->adapter->index,
			(bus_err_notif) ? "_NOTIF" : "",
			bus_err_txt[err->type],
			(err->rx) ? "Rx" : "Tx", err->code,
			(err->gen) ? 'y' : 'n');

		/* fallthrough */
		fallthrough;
	case PCANFD_BERRLOG_LEVEL_NONE:
		break;
	}
}

void pcan_handle_error_internal(struct pcandev *dev, struct pcanxl_rxmsg *rx,
				int err_internal)
{
#if defined(DEBUG_TRACE)
	pr_info(DEVICE_NAME ": %s(pcan%d, err_internal=%d)\n",
		__func__, dev->nMinor, err_internal);
#endif
	...
	case PCANFD_RX_OVERFLOW:
		if (pcan_set_status_bit(dev, CAN_ERR_OVERRUN))
			pr_err(DEVICE_NAME "%u: Rx fifo overflow\n",
			       dev->nMinor);
		break;
	case PCANFD_TX_OVERFLOW:
		if (pcan_set_status_bit(dev, CAN_ERR_XMTFULL))
			pr_err(DEVICE_NAME "%u: Tx fifo overflow\n",
			       dev->nMinor);
		break;
	default:
		pcan_set_status_bit(dev, CAN_ERR_RESOURCE);
		pr_err(DEVICE_NAME "%u: internal error %d\n",
	...
	/* TODO: add counters of msg lost because of fifo full */
}
...
{
#if defined(DEBUG_TRACE)
	pr_info(DEVICE_NAME ": %s(pcan%d, err_protocol=%d)\n",
		__func__, dev->nMinor, err_protocol);
#endif
	...
	case PCANFD_RX_OVERFLOW:
		...
		pr_err(DEVICE_NAME "%u: protocol Rx overflow\n", dev->nMinor);
		break;
	case PCANFD_TX_OVERFLOW:
		if (!pcan_set_status_bit(dev, CAN_ERR_QXMTFULL))
			return;
		pr_err(DEVICE_NAME "%u: protocol Tx overflow\n", dev->nMinor);
		break;
	default:
		pcan_set_status_bit(dev, CAN_ERR_RESOURCE);
		pr_err(DEVICE_NAME "%u: protocol error %d\n",
...
void pcan_cleanup_dev(struct pcandev *dev)
{
	...
	pr_info(DEVICE_NAME ": %s(dev=%p flags=%08Xh): cleaning pcan%d\n",
		 __func__, dev,
		 (dev) ? dev->flags : 0,
		 (dev) ? dev->nMinor : -1);
#endif
...
#else
			(long )dev->total_stats.rx.frames,
			(long )dev->total_stats.tx.frames,
#endif
			dev->total_stats.rx_irq_counter +
...
/* CentOS 9/RHEL v9 runs 5.14.0 but also need const qualifier */
#ifdef PCAN_SYSFS_ATTR_NEEDS_CONST
...
struct pcandev *__pcan_free_dev(struct pcandev *dev)
{
	...
	dev->icache = pcan_free(dev->icache);
...
int pcan_get_dev_features(struct pcandev *dev, u32 *features)
{
	*features = 0;
	...
	if (dev->features & PCAN_DEV_ERR_GEN_RDY)
		*features |= PCANFD_FEATURE_ERR_GEN;
#endif
	if (dev->features & PCAN_DEV_XL_RDY)
		*features |= PCANXL_FEATURE_XL;
	if (dev->features & PCAN_DEV_FD_RDY)
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(u_size=%d k_size=%u)\n",
		__func__, opt->size, (uint )sizeof(*dev->fd_bittiming_caps));
#endif

	if (!dev->fd_bittiming_caps)
		return -EOPNOTSUPP;

	opt->size = sizeof(*dev->fd_bittiming_caps);
	if (pcan_copy_to_user(opt->value, dev->fd_bittiming_caps, opt->size, c)) {
		pr_err(DEVICE_NAME ": %s(): copy_to_user() failure\n",
			__func__);
		return -EFAULT;
	}
			     ...
			     struct pcanfd_option *opt, void *c)
{
	...
	/* Trick: if opt->name != PCANFD_OPT_SELF_ACK then
	 * this is an internal call: value is not a userspace address.
	 */
	if (opt->name != PCANFD_OPT_SELF_ACK)
		memcpy(&tmp, opt->value, sizeof(tmp));

	else if (pcan_copy_from_user(&tmp, opt->value, sizeof(tmp), c)) {
		pr_err(DEVICE_NAME ": %s(): copy_from_user() failure\n",
			__func__);
		return -EFAULT;
	}
	...
	if (tmp) {
		dev->flags &= ~PCAN_DEV_SELF_ACK;
		err = canfd_set_options(dev, CANFD_OPTION_SELF_ACK, 0);
		if (!err)
			dev->flags |= PCAN_DEV_SELF_ACK;
		else
			pr_err(DEVICE_NAME "%d: %s CAN%u %u: "
			       "failed to set slf-ack (err %d)\n",
			       dev->nMinor, dev->adapter->name,
		       pcan_idx(dev)+1, dev->adapter->index, err);
	} else {
		dev->flags |= PCAN_DEV_SELF_ACK;
		err = canfd_clr_options(dev, CANFD_OPTION_SELF_ACK, 0);
		if (!err)
			dev->flags &= ~PCAN_DEV_SELF_ACK;
		else
			pr_err(DEVICE_NAME "%d: %s CAN%u %u: "
			       "failed to clear slf-ack (err %d)\n",
			       dev->nMinor, dev->adapter->name,
			       pcan_idx(dev)+1, dev->adapter->index, err);
	}
			    ...
			    struct pcanfd_option *opt, void *c)
{
	...
	if (tmp) {
		dev->flags &= ~PCAN_DEV_BRS_IGN;
		err = canfd_set_options(dev, CANFD_OPTION_BRS_IGN, 0);
		if (!err)
			dev->flags |= PCAN_DEV_BRS_IGN;
		else
			pr_err(DEVICE_NAME "%d: %s CAN%u %u: "
			       "failed to set brs-ign (err %d)\n",
			       dev->nMinor, dev->adapter->name, pcan_idx(dev)+1,
			       dev->adapter->index, err);
	} else {
		dev->flags |= PCAN_DEV_BRS_IGN;
		err = canfd_clr_options(dev, CANFD_OPTION_BRS_IGN, 0);
		if (!err)
			dev->flags &= ~PCAN_DEV_BRS_IGN;
		else
			pr_err(DEVICE_NAME "%d: %s CAN%u %u: "
			       "failed to clear brs-ign (err %d)\n",
			       dev->nMinor, dev->adapter->name, pcan_idx(dev)+1,
			       dev->adapter->index, err);
	}
...
#ifdef PCANFD_FEATURE_ERR_GEN
static int pcan_set_err_gen(struct pcandev *dev,
			    struct pcanfd_option *opt, void *c)
{
	struct pcanfd_error_generator tmp;
	...
	return canfd_handle_error_generator_option(dev, &tmp);
}
#endif
...
int __pcan_set_ts_mode(struct pcandev *dev, int ts_mode, bool fallback)
{
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(ts_mode=%d, fallback=%d)\n",
	 ...
	 * Note: up to now, the TS_SOF feature is available in CANFD/CANXL FW
	 * which use the same option management API
	 */
	if (dev->features & PCAN_DEV_TS_SOF_RDY) {
		if (sof_flag) {
			if (!canfd_set_options(dev, CANFD_OPTION_TS_SOF, 0))
				dev->flags |= PCAN_DEV_TS_SOF;

		} else {
			if (!canfd_clr_options(dev, CANFD_OPTION_TS_SOF, 0))
				dev->flags &= ~PCAN_DEV_TS_SOF;
		}
					...
					struct pcanfd_option *opt, void *c)
{
	...
	return __pcan_set_ts_mode(dev, tmp, false);
}
#endif
 ...
 * CAN-XL options
 */
static int pcan_get_xl_bittiming_range(struct pcandev *dev,
				       struct pcanfd_option *opt, void *c)
{
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(u_size=%d k_size=%u)\n",
		__func__, opt->size, (uint )sizeof(*dev->xl_bittiming_caps));
#endif

	if (!dev->xl_bittiming_caps)
		return -EOPNOTSUPP;

	opt->size = sizeof(*dev->xl_bittiming_caps);
	if (pcan_copy_to_user(opt->value, dev->xl_bittiming_caps, opt->size, c)) {
		pr_err(DEVICE_NAME ": %s(): copy_to_user() failure\n",
			__func__);
		return -EFAULT;
	}
...
static int pcan_get_xl_pwm_caps(struct pcandev *dev,
				struct pcanfd_option *opt, void *c)
{
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(u_size=%d k_size=%u)\n",
		__func__, opt->size, (uint )sizeof(*dev->xl_pwm_caps));
#endif

	if (!dev->xl_pwm_caps)
		return -EOPNOTSUPP;

	opt->size = sizeof(*dev->xl_pwm_caps);
	if (pcan_copy_to_user(opt->value, dev->xl_pwm_caps, opt->size, c)) {
		pr_err(DEVICE_NAME ": %s(): copy_to_user() failure\n",
			__func__);
		return -EFAULT;
	}
...
static struct pcanfd_options pcan_def_opts[PCANXL_OPT_MAX] =
{
...
#ifdef PCANFD_FEATURE_ERR_GEN
	[PCANFD_OPT_ERR_GEN] = {
		.req_size = sizeof(struct pcanfd_error_generator),
		.set = pcan_set_err_gen,
	},
#endif
	/* CANXL options */
	[PCANXL_OPT_XL_BITTIMING_RANGES] = {
		.req_size = sizeof(struct pcanfd_bittiming_range),
		.get = pcan_get_xl_bittiming_range,
	},
	[PCANXL_OPT_XL_PWM_RANGES] = {
		.req_size = sizeof(struct pcanxl_pwm_range),
		.get = pcan_get_xl_pwm_caps,
	},
};
 ...
 * (CANFD API) device_open_fd() entry point.
 */
				       ...
				       u8 ext, u8 listen_only)
{
	...
	pr_info(DEVICE_NAME
		"%d: %s(btr0btr1=%04xh, ext=%u, listen_only=%u)\n",
		dev->nMinor, __func__, btr0btr1, ext, listen_only);
#endif
	if (!dev->device_open_fd) {
		pr_err(DEVICE_NAME ": ABNORMAL NULL device_open_fd callback\n");
		return -EFAULT;
	}
...
/* CAN-FD API entry point.
 */
static int pcan_device_open_xl_wrapper(struct pcandev *dev,
				       struct pcanfd_init *fdi)
{
	struct pcanxl_init xli = {};

	if (!dev->device_open_xl) {
		pr_err(DEVICE_NAME ": ABNORMAL NULL device_open_xl callback\n");
		return -EFAULT;
	}

	return dev->device_open_xl(dev, pcanfd_init_to_xl(dev, &xli, fdi));
}
...
int pcan_icache_alloc(struct pcandev *dev, int slot_count)
{
	...
	size_of = sizeof(struct pcan_icache) +
		      slot_count * sizeof(struct pcan_frm_counter);

	dev->icache = pcan_malloc(size_of, GFP_KERNEL);
	if (!dev->icache) {
		pr_warn(DEVICE_NAME "%u: no mem for internal cache\n",
			dev->nMinor);
		return -ENOMEM;
	}

	memset(dev->icache, '\0', size_of);

	dev->icache->size = slot_count;
...
void pcan_icache_in(struct pcandev *dev, int frm, int bytes)
{
	...
	dev->icache->rec[dev->icache->slot].frames = frm;
	dev->icache->rec[dev->icache->slot].bytes = bytes;

	dev->icache->slot = (dev->icache->slot + 1) % dev->icache->size;
}
...
{
	if (dev->icache)
		memset(dev->icache->rec, '\0',
		       sizeof(struct pcan_frm_counter) * dev->icache->size);
}
			...
			u32 features)
{
	...
	/* v9: moved to pcanxl_dev_open_init()
	 * __pcan_set_ts_mode(dev, deftsmode, true);
	 */
	...
	/* do the same for fd_bitrate and xl_bitrate */
	dev->def_init_settings.fd_data.bitrate = pcan_def_fd_bitrate;
	dev->def_init_settings.xl_data.bitrate = pcan_def_xl_bitrate;
	dev->fd_bittiming_caps = NULL;
	 ...
	 * init device_open() callback with wrapper to device_open_fd()
	 * callback. Old controllers will then overload it with their own.
	 * Same for CAN FD controllers: init device_open_fd() with wrapper to
	 * device_open_xl(). CAND controllers will then overlmoad it with their
	 * own.
	 * Newer (CAN XL) will then just have to provide their own
	 * device_open_xl() only.
	 */
	dev->device_open = pcan_device_open_fd_wrapper;
	dev->device_open_fd = pcan_device_open_xl_wrapper;
...
void pcanfd_dump_bittiming(struct pcan_bittiming *pbt, u32 clock_Hz)
{
	...
	pr_info(DEVICE_NAME ": clk=%uMHz btr=%u%cbps (%u) sp=%u "
		"[BRP=%u TSEGx=%u/%u SJW=%u TQ=%u]\n",
		clock_Hz/1000000, btr, unit, pbt->bitrate, pbt->sample_point,
...
static int __init pcan_init_module(void)
{
	...
	pcan_gettimeofday_ns(&pcan_drv.sInitTime);
	...
	if (!pcan_drv.sInitTime.tv_sec && !pcan_drv.sInitTime.tv_nsec)
		pr_warn(DEVICE_NAME ": WARNING: got abnormal NULL time\n");

	/* in this version, "bitrate" parameter (and "dbitrate" parameter)
	...
	if (dbitrate)
		strtounit(dbitrate, &pcan_def_fd_bitrate, "kM");

	if (xbitrate)
		strtounit(xbitrate, &pcan_def_xl_bitrate, "kM");

#ifdef PCAN_DEV_RXQSIZE_MIN
	/* check whether Rx/Tx queue default sizes are ok */
	if (rxqsize < PCAN_DEV_RXQSIZE_MIN) {
		pr_info(DEVICE_NAME
			": rxqsize(%u) fixed to its min value (%u)\n",
	...
	if (rxqsize > PCAN_DEV_RXQSIZE_MAX) {
		pr_info(DEVICE_NAME
			": rxqsize(%u) fixed to its max value (%lu)\n",
	...
	if (txqsize < PCAN_DEV_TXQSIZE_MIN) {
		pr_info(DEVICE_NAME
			": txqsize(%u) fixed to its min value (%u)\n",
	...
	if (txqsize > PCAN_DEV_TXQSIZE_MAX) {
		pr_info(DEVICE_NAME
			": txqsize(%u) fixed to its max value (%lu)\n",
...
#ifdef DEBUG
	pr_info(DEVICE_NAME ": driver start time=%lld.%09lld s.\n",
		pcan_drv.sInitTime.tv_sec,
		pcan_drv.sInitTime.tv_nsec);
