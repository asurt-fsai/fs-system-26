...
int pcan_make_output(char *buffer, struct pcanxl_msg *pf)
{
	char *ptr = buffer;
	char r_or_m = '?', s_or_e = '?';
	int i;
	...
	if (pf->flags & PCANFD_TIMESTAMP) {
		...
		ms += pf->timestamp.tv_sec * MSEC_PER_SEC;

		/* print timestamp */
		ptr += sprintf(ptr, " %11lld %03lu", ms, ns / NSEC_PER_USEC);
	}
...
int pcan_parse_input_message(char *buffer, struct pcanxl_msg *pf)
{
	char *ptr = buffer;
	u32 dwLen;
	u32 dwDat;
	 ...
	 */
	if (dwLen > PCANFD_CANFD_MAXDATALEN)
		pf->type = PCANXL_TYPE_CANXL;
	else if (dwLen > PCANFD_CAN20_MAXDATALEN)
		pf->type = PCANFD_TYPE_CANFD_MSG;
	else
		pf->type = PCANFD_TYPE_CAN20_MSG;

	if (pf->data_len < dwLen)
		goto reject;

	pf->data_len = (u16)dwLen;
 ...
 *	"data_bitrate="
 */
static int scan_bitrate_string(char **buffer, struct pcanxl_init *pxli)
{
	char *pc;
	...
	for (pc = *buffer; !is_eow(*pc); ) {
		...
		*pe = '\0';
		...
		if (!strcmp(pc, "f_clock"))
			...
			pv = &pxli->nominal.bitrate;
		else if (!strcmp(pc, "nom_brp"))
			pv = &pxli->nominal.brp;
		else if (!strcmp(pc, "nom_tseg1"))
			pv = &pxli->nominal.tseg1;
		else if (!strcmp(pc, "nom_tseg2"))
			pv = &pxli->nominal.tseg2;
		else if (!strcmp(pc, "nom_sjw"))
			pv = &pxli->nominal.sjw;
		else if (!strcmp(pc, "data_bitrate") || !strcmp(pc, "fd_bitrate"))
			pv = &pxli->fd_data.bitrate;
		else if (!strcmp(pc, "data_brp") || !strcmp(pc, "fd_brp"))
			pv = &pxli->fd_data.brp;
		else if (!strcmp(pc, "data_tseg1") || !strcmp(pc, "fd_tseg1"))
			pv = &pxli->fd_data.tseg1;
		else if (!strcmp(pc, "data_tseg2") || !strcmp(pc, "fd_tseg2"))
			pv = &pxli->fd_data.tseg2;
		else if (!strcmp(pc, "data_sjw") || !strcmp(pc, "fd_sjw"))
			pv = &pxli->fd_data.sjw;
 ...
 */
int pcan_parse_input_init(char *buffer, struct pcanxl_init *pxli)
{
	char *ptr = buffer;
	u32 btr0btr1;
	int i, err = -EINVAL;
	...
	if (!scan_unsigned_number(&ptr, &btr0btr1)) {
		...
		pcan_btr0btr1_to_bittiming(&pxli->nominal, btr0btr1);

	} else if (scan_bitrate_string(&ptr, pxli))
		goto reject;
	...
	if (pxli->fd_data.bitrate || pxli->fd_data.brp || pxli->fd_data.tseg1 ||
	    pxli->fd_data.tseg2 || pxli->fd_data.sjw)
