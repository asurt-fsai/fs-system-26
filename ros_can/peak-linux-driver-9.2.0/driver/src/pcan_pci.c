...
static int pci_msi_vec_count(struct pci_dev *pdev)
{
	u16 msi_msgctl;
	int ret, msi_base;

	/* read board MSI capacity */
	msi_base = pci_find_capability(pdev, PCI_CAP_ID_MSI);
	if (!msi_base) {
		dev_err(&pdev->dev, "pci_find_capability() failure\n");
		return -EIO;
	}

	/* read board MSI maximum of vectors available */
	ret = pci_read_config_word(pdev, msi_base + PCI_MSI_FLAGS, &msi_msgctl);
	if (!ret)

		/* always request the count of vectors exported by the board */
		ret = 1 << ((msi_msgctl & PCI_MSI_FLAGS_QMASK) >> 1);
...
int pcan_pci_enable_msi(struct pcan_pci_adapter *pa, int can_count, int irq_max)
{
	 ...
	 */
	if (irq_max < 0)
		irq_max = pci_msi_vec_count(pa->dev);
	 ...
	 */
	pa->msi_count = pcan_pci_enable_msi_range(pa->dev, can_count, irq_max);

#ifdef DEBUG_MSI
	pr_info(DEVICE_NAME ": enabling [%u..%u] MSI status: %d\n",
		can_count, irq_max, pa->msi_count);
#endif
	if (pa->msi_count <= 0) {
...
static struct pcanfd_options pcan_pci_options[PCANXL_OPT_MAX] =
{
