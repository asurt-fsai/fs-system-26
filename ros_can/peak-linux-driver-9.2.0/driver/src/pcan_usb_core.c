...
#define DEBUG_TRACE
#define DEBUG_DECODE
#define DEBUG_TX_FULL
#define DEBUG_RX_FULL
#endif
#endif

#if LINUX_VERSION_CODE <= KERNEL_VERSION(2, 6, 18) || \
    LINUX_VERSION_CODE >= KERNEL_VERSION(6, 8, 0)
#ifndef PCAN_USB_DOESNT_NEED_DRVWRAP
#define PCAN_USB_DOESNT_NEED_DRVWRAP
#endif
#endif
...
/* Size MUST include the size of the zero USB packet the FW adds to each
 * response it gives. CANFD and older devices do need more than 512 bytes,
 * (that is, 512 + 4 or 8) therefore 1024 was used.
 *
 * According to PCAN-USB_XL_3rd_party.xls, the driver should request a 4*1024
 * bytes large buffer for the command replies (so does the Win driver)
 */
#define PCAN_USB_CMD_BUFFER_SIZE	1024
#define PCAN_USBXL_CMD_BUFFER_SIZE	(4*1024)

/* Defines the size of one USB message that can be received from the device
 * Driver allocates one buffer of 2 x read_buffer_size to optimize
 * consecutive reads
 */
...
/* PCAN-USB_XL_3rd_party.xlsx says "12x1024" but Win driver uses 8x1024 */
#define PCAN_USBXL_READ_BUFFER_SIZE	(8*1024)
...
/* PCAN-USB_XL_3rd_party.xlsx says "5x512", Win driver uses 8x512 */
#define PCAN_USBXL_WRITE_BUFFER_SIZE	(5*512)
...
static struct usb_device_id pcan_usb_ids[] = {
	...
	{ USB_DEVICE(PCAN_USB_VENDOR_ID, PCAN_USBXL_PRODUCT_ID) },
	{ }	/* Terminating entry */
};
...
#endif
{
	...
	int err = urb->status;
	u32 now_ms = pcan_get_now_ms();

#if defined(DEBUG_TRACE) || defined(DEBUG_TX)
	pr_info(DEVICE_NAME
		"%u: %s() status=%d actual_length=%d w_active_urbs=%u\n",
		dev->nMinor, __func__, err, urb->actual_length,
	...
	usb_if->rtt_ms = now_ms - usb_if->ts_submit_time_ms;

	/* don't count interrupts - count packets */
	dev->total_stats.tx_irq_counter++;
	dev->session_stats.tx_irq_counter++;
	...
	case 0:
		...
		if (!(dev->flags & PCAN_DEV_CLOSING)) {
			...
			/* stop signaling when Tx fifo is almost full */
...
lbl_unplug:
	if (!usb_if->removing_driver)
		pr_err(DEVICE_NAME ": err %d when reading: "
			"is %s #%u usb cable disconnected?\n",
			urb->status, usb_if->adapter->name,
			usb_if->adapter->index);
...
static int pcan_usb_write(struct pcandev *dev, struct pcanusr *ctx)
{
	...
	int write_buffer_size, err = 0;
	struct pcan_frm_counter tx_prev = dev->session_stats.tx;
	...
	if (write_buffer_size > 0) {
		...
		/* PCAN-USB XL: true for data len = [2001..2004] */
		if (!(urb->transfer_buffer_length % u->pipe_write.wDataSz)) {
#ifdef DEBUG_TX_FULL
			pr_info(DEVICE_NAME "%u: %u multiple of %u: "
				"ZLP added!\n", dev->nMinor, urb->transfer_buffer_length,
				u->pipe_write.wDataSz);
#endif
			urb->transfer_flags |= URB_ZERO_PACKET;
		}
		...
		err = __usb_submit_urb(urb);
		if (!err) {
			usb_if->ts_submit_time_ms = pcan_get_now_ms();

			pcan_icache_in(dev, dev->session_stats.tx.frames -
					    tx_prev.frames,
		...
		pr_err(DEVICE_NAME
		       "%u: failed to submit %d bytes packet to %s CAN%u "
		       "(err %d)\n", dev->nMinor, write_buffer_size,
...
static int pcan_usb_init_urb(struct urb **urb)
{
...
static int pcan_usb_alloc_resources(struct pcan_usb_interface *usb_if)
{
	...
	USB_PORT *u;
	int err = 0, c;
	int def_cmd_bsize = PCAN_USB_CMD_BUFFER_SIZE;

#ifdef DEBUG_TRACEdef_cmd_bsize
	...
	case PCAN_USBXL_PRODUCT_ID:
		/* Note: works at USB High-Speed only! If speed negociation
		 *       ends in Full-Speed then device disconnects
		 *       automatically
		 *       EPO max packet size is 64 bytes
		 */
		if (usb_if->usb_dev->speed < USB_SPEED_HIGH)
			pr_warn(DEVICE_NAME
				": device can't work at USB speed %u!\n",
			       	usb_if->usb_dev->speed);

		def_cmd_bsize = PCAN_USBXL_CMD_BUFFER_SIZE;
		usb_if->read_buffer_size = PCAN_USBXL_READ_BUFFER_SIZE;
		usb_if->write_buffer_size = PCAN_USBXL_WRITE_BUFFER_SIZE;
	...
	for (c = 0; c < usb_if->can_count; c++) {
		...
		/* Use greater than 512 bytes so that FW could add its
		 * "end-of-collection" record (4 or 8 bytes) in its response.
		 */
		u->cout_bsize = def_cmd_bsize;
		u->cout_baddr = pcan_malloc(u->cout_bsize, GFP_KERNEL|GFP_DMA);
		if (!u->cout_baddr) {
			err = -ENOMEM;
			goto fail;
		}
...
static int pcan_usb_stop(struct pcandev *dev)
{
...
#if defined(DEBUG_TRACE) || defined(DEBUG_TX)
	pr_info(DEVICE_NAME "%u: %s(CAN%u): flags=%08xh\n",
		dev->nMinor, __func__, pcan_idx(dev)+1, dev->flags);
#endif
...
#ifdef DEBUG_TX
	pr_info(DEVICE_NAME "%u: have still %d+%d active URBs on interface\n",
		dev->nMinor,
	 ...
	 * being ACKed...
	 *
	 * Note: this loop can take up to 50 ms for the PCAN-USB (FW 2.8)
	 */
	for (i = 0; i < 10; i++)
	...
	if (i >= 10) {
		pr_warn(DEVICE_NAME
			"%u: too long to wait for URB to be released "
			"(still %u active)\n",
			dev->nMinor, atomic_read(&usb_if->w_active_urbs));
		//? return -EBUSY;
	} else {
...
/*
 * static int __pcan_usb_device_open_init(struct pcandev *dev)
 */
static int __pcan_usb_device_open_init(struct pcandev *dev)
{
	...
	pcan_icache_reset(dev);

	return 0;
}

/*
 * static int __pcan_usb_device_open_end(struct pcandev *dev)
 */
static int __pcan_usb_device_open_end(struct pcandev *dev)
{
	struct pcan_usb_interface *usb_if = pcan_usb_get_if(dev);
                                ...
                                u8 bExtended, u8 bListenOnly)
{
	...
	pr_info(DEVICE_NAME ": %s(pcan%u): nOpenPaths=%d\n",
		__func__, dev->nMinor, dev->nOpenPaths);
#endif
	err = __pcan_usb_device_open_init(dev);
	if (err)
		return err;
	...
	return __pcan_usb_device_open_end(dev);
}
...
/* CANXL API open path */
static int pcan_usb_device_open_xl(struct pcandev *dev,
				   struct pcanxl_init *pxli)
{
	...
	/* init hardware specific parts */
	err = usb_if->device_ctrl_open_xl(dev, pxli);
	if (err)
		return err;
...
static void pcan_usb_device_release(struct pcandev *dev)
{
	 ...
	 * anymore.
	 *
	 * Calls bus_off() device callback at the end.
	 */
	pcan_usb_stop(dev);
}
...
static struct pcanfd_options pcan_usb_opts[PCANXL_OPT_MAX] =
{
...
static struct pcanfd_options pcan_usb_chip_opts[PCANXL_OPT_MAX] =
{
                               ...
                               int ctrl_index)
{
	...
	case PCAN_USBXL_PRODUCT_ID:
		canxl_soft_init(dev, &usb_if->hw_ver);

		/* CANXL core cache size = 4KB ;
		 * which is ~2 x PCAN_USBXL_WRITE_BUFFER_SIZE
		 */
		icache_size = 2;
		break;
	...
	case PCAN_USBPROFD_PRODUCT_ID:
	case PCAN_USBX6_PRODUCT_ID:
		canfd_soft_init(dev, &usb_if->hw_ver);
		...
		/* CANFD core cache size = 2KB ;
		 * which is 4 x PCAN_USBFD_WRITE_BUFFER_SIZE
		 */
		icache_size = 4;
		break;
	...
	/* allocate internal cache struct to keep a trace of the last
	 * frames writtent to the device.
	 */
	pcan_icache_alloc(dev, icache_size);
	...
	/* device_init() has been called: can do that! */
	 ...
	 */
	if (usb_if->device_ctrl_open_xl)
		dev->device_open_xl = pcan_usb_device_open_xl;
	else if (usb_if->device_ctrl_open_fd)
                           ...
                           const struct usb_device_id *id)
{
	...
	for (i = 0; i < dev_ctrl_count; i++ ) {
		...
		/* increment can_count before writing devs[] so that
		 * __counted_by(can_count) keeps UBSAN happy
		 */
		usb_if->can_count++;
...
#ifdef DEBUG_USB
	pr_info(DEVICE_NAME ": "
		"CAN EP CMD[IN=%02xh OUT=%02xh] DATA[IN=%02xh OUT0=%02xh...]\n",
...
#ifdef DEBUG_TRACE
	pr_info(DEVICE_NAME ": %s(usb_if=%p, can_count=%zu)\n",
...
/* CentOS 9/RHEL v9 runs 5.14.0 but don't need drvwrap */
#ifdef PCAN_USB_DOESNT_NEED_DRVWRAP
