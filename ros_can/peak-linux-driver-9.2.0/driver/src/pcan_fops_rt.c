			       ...
			       struct pcanusr *ctx, TPCANMsg __user *usr)
{
	...
	err = pcanxl_ioctl_send_msg(dev, (struct pcanxl_txmsg *)&tx, NULL, ctx);
	if (err)
		goto fail;
			      ...
			      struct pcanusr *ctx, TPCANRdMsg __user *usr)
{
	...
	if (copy_to_user_rt(user_info, usr,
			    copy_to_cc(&msg, (struct pcanxl_msg *)&rx.msg),
			    sizeof(*usr))) {
			 ...
			 IOCTL_REQUEST_TYPE cmd, void *arg)
{
	...
	case PCANXL_SET_INIT:
		err = copy_from_user_rt(user_info, &xli, up, sizeof(xli));
		if (err) {
			pr_err(DEVICE_NAME
				": %s(%u): copy_from_user_rt() failure\n",
				__func__, __LINE__);
	...
	case PCANXL_GET_INIT:
		...
		err = copy_to_user_rt(user_info, up, &xli, _IOC_SIZE(cmd));
		if (err) {
			pr_err(DEVICE_NAME
			       ": %s(%u): copy_to_user_rt() failure\n",
			       __func__, __LINE__);
			return pcan_put_dev(dev, -EFAULT);
		}
		break;
	...
	case PCANFD_SEND_MSG:
		err = copy_from_user_rt(user_info, &fdm, up, sizeof(fdm));
		if (err) {
			pr_err(DEVICE_NAME
				": %s(%u): copy_from_user_rt() failure\n",
				__func__, __LINE__);
		...
		err = pcanfd_ioctl_send_msg(dev, &fdm, ctx);
		break;

	case PCANFD_RECV_MSG:
		err = pcanfd_ioctl_recv_msg(dev, &fdm, ctx);
		if (err)
			break;

		err = copy_to_user_rt(user_info, up, &fdm, sizeof(fdm));
		if (err) {
			pr_err(DEVICE_NAME
				": %s(%u): copy_to_user_rt() failure\n",
				__func__, __LINE__);
	...
	default:
		err = handle_pcan_flex_cmd(cmd, dev, up, ctx, NULL);
		break;
	}
