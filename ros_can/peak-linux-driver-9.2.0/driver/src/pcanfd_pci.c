...
/* if defined, raw spinlock is used to ensure atomicity of the written cmd */
//#define PCAN_CMD64_USES_RAW_SPIN_LOCK
...
#define CANFD_MISC_TS_RST		0x00000001	/* timestamp cnt reset*/
...
#ifdef PCAN_CMD64_USES_RAW_SPIN_LOCK
				...
				const char *prompt, int offset, int len)
{
	...
	for (i = 0; i <= len; ) {
		if (!(i % 16))
			l = scnprintf(tmp, sizeof(tmp), DEVICE_NAME ": %03x: ",
				      offset + i);
				 ...
				 int offset, int len)
{
	...
	for (i = 0; i < len; ) {
		if (!(i % 16))
			l = scnprintf(tmp, sizeof(tmp), ": %03x - ",
				      offset + i);
...
static void pcifd_wait_for_eot(struct pcandev *dev)
{
	...
	/* make several dummy reads/writes to sync */
	for (i = 0; i < 8; i++)
		ucan_sys_writel(ucan_pci,
				ucan_sys_readl(ucan_pci, PCIEFD_REG_VER1) + i,
				PCIEFD_REG_VER1);
}
...
static int __pcifd_device_write(struct pcandev *dev, struct pcanusr *ctx)
{
	...
	while (1) {
		...
		err = canfd_encode_txmsg(dev, page->vbase + page->offset,
						page->size - page->offset);
		if (err < 0) {
	...
	pcan_icache_in(dev, dev->session_stats.tx.frames - tx_prev.frames,
			    dev->session_stats.tx.bytes - tx_prev.bytes);
	...
	if (frc > 0) {
		pr_info(DEVICE_NAME
			...
			frc, dev->session_stats.tx.frames, err);
		pcifd_tx_dma_dump(dev, __func__, offset, len);
	} else {
		pr_info(DEVICE_NAME
...
static irqreturn_t pcifd_irq_handler(int irq, void *arg)
{
	...
	pr_info(DEVICE_NAME ": %s(CAN%u) IRQ%u dev=%p\n",
		__func__, dev->can_idx+1, dev->wIrq, dev);
#endif
...
static void pcifd_device_release(struct pcandev *dev)
{
	...
	/* Note: DON't canfd_tx_abort() when Tx DMA NOT in RST mode! */

	/* bus off */
	canfd_set_bus_off(dev);

	/* Note: v8.16 uses canfd_device_close() to clear options set */
	canfd_device_close(dev);
	...
	canfd_tx_abort(dev, CANFD_TX_ABORT_FLUSH);
}
...
#if defined(DEBUG_TRACE) || defined(DEBUG_IRQ_RX) || defined(DEBUG_IRQ_TX)
	pr_info(DEVICE_NAME ": %s(CAN%u) IRQ%u irq_flags=%0xh dev=%p\n",
		__func__, dev->can_idx+1, dev->wIrq, irq_flags, dev);
#endif
				...
				struct pcanfd_init *pfdi)
{
	...
	if (dev->bus_state == PCANFD_ERROR_BUSOFF) {
		...
		/* reset counters only */
		canfd_clr_err_counters(dev);

		pcan_set_tx_engine(dev, TX_ENGINE_STARTED);

		err = canfd_set_bus_on(dev);
		if (err) {
			pr_err(DEVICE_NAME
	...
	err = canfd_device_open_fd(dev, pfdi, 0, 0);
	if (err) {
		pr_err(DEVICE_NAME
...
static struct pcan_timespec *pcifd_decode_timestamp(struct pcandev *dev,
...
/* handle a CANFD_MSG_CAN_RX message read from Rx DMA */
static int pcifd_handle_canrx(struct ucan_engine *ucan,
			      void *rx_msg, void *arg)
{
	struct canfd_rx_msg *rm = (struct canfd_rx_msg *)rx_msg;
	struct pcandev *dev = (struct pcandev *)arg;
	struct pcan_timespec tv;

	return canfd_post_rxmsg(dev, rm,
		pcifd_decode_timestamp(dev, le32_to_cpu(rm->ts_low),
					le32_to_cpu(rm->ts_high), &tv));
}

/* handle a CANFD_MSG_ERROR message recieved from Rx DMA area */
static int pcifd_handle_error(struct ucan_engine *ucan,
			      void *rx_msg, void *arg)
{
	struct canfd_error_msg *er = (struct canfd_error_msg *)rx_msg;
	struct pcandev *dev = (struct pcandev *)arg;
	struct pcan_timespec tv;
	...
	return canfd_post_error(dev, er,
		pcifd_decode_timestamp(dev, le32_to_cpu(er->ts_low),
				       le32_to_cpu(er->ts_high), &tv));
}

/* handle a CANFD_MSG_BUSLOAD message received from Rx DMA area */
static int pcifd_handle_bus_load(struct ucan_engine *ucan,
				 void *rx_msg, void *arg)
{
	struct canfd_bus_load_msg *bl = (struct canfd_bus_load_msg *)rx_msg;
	struct pcandev *dev = (struct pcandev *)arg;
	struct pcan_timespec tv;

	return canfd_post_busload(dev, bl,
			pcifd_decode_timestamp(dev, le32_to_cpu(bl->ts_low),
						le32_to_cpu(bl->ts_high), &tv));
}

/* handle a CANFD_MSG_STATUS message recieved from Rx DMA area */
static int pcifd_handle_status(struct ucan_engine *ucan,
			       void *rx_msg, void *arg)
{
	struct canfd_status_msg *st = (struct canfd_status_msg *)rx_msg;
	struct pcandev *dev = (struct pcandev *)arg;
	int err;
	...
	pr_info(DEVICE_NAME ": %s(CAN%u): EP=%u EW=%u BO=%u RB=%u\n",
		__func__, dev->can_idx+1, !!CANFD_STMSG_PASSIVE(st),
		!!CANFD_STMSG_WARNING(st), !!CANFD_STMSG_BUSOFF(st),
		!!CANFD_STMSG_RB(st));
#endif
...
static int (*pcifd_handlers[])(struct ucan_engine *, void *, void *) = {
	[CANFD_MSG_CAN_RX] = pcifd_handle_canrx,

	/* this handler pushes STATUS msgs when rx/tx errors counters change */
	[CANFD_MSG_ERROR] = pcifd_handle_error,
	[CANFD_MSG_STATUS] = pcifd_handle_status,
	[CANFD_MSG_BUSLOAD] = pcifd_handle_bus_load,
};
...
static int pcifd_dev_probe(struct pcifd_adapter *ucan_pci, int c)
{
	...
	canfd_soft_init(dev, &ucan_pci->pci.adapter.hw_ver);

	/* allocate internal cache struct to keep a trace of the last
	 * frames writtent to the device:
	 *
	 * PCIEFD_LNK_COUNT (2) pages can be in-flight
	 */
	pcan_icache_alloc(dev, PCIEFD_LNK_COUNT);
	...
	pcifd_dev_dump(dev, __func__, 0, -1);
#endif
	/* init uCAN engine: here, the uCAN handles one CANFD pcandev */
		     ...
		     int can_count)
{
	...
	else if (ucan_pci->pci.msi_count >= can_count)
		fdusemsi = PCAN_PCI_USEMSI_NOTSHARED;
	else
		fdusemsi = PCAN_PCI_USEMSI_SHARED;
#endif /* PCAN_PCI_ENABLE_MSI */

	/* check module parameters; warn if out-of-range and use default one */
	...
	for (i = 0; i < can_count; i++)
		ucan_x_writel(pcifd_dev(ucan_pci, i),
				CANFD_MISC_TS_RST, PCIEFD_REG_MISC);
