...
#
# 1.1.0 - Stephane Grosjean <stephane.grosjean@hms-networks.com>
# - add cat /boot/config*
#
VERSION=1.1.0
...
if [ -f $config ]; then
	...
	run $ZCAT /proc/config.gz
elif [ -f /boot/config ]; then
	run $CAT /boot/config
else
	echo "Can't get current kernel config from $config nor /proc/config.gz nor /boot/config, sorry..."
fi
...
echo "Please send $LOG to <support.peak@hms-networks.com>"
exit 0
