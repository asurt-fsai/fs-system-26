...
LINUX_API_VISIBILITY TPCANStatus CAN_InitializeXL(
    TPCANHandle Channel,
    TPCANBitrateXL BitrateXL)
{
	...
	PCB_MUTEX_LOCK(&g_mut);
	/* logging */
	pcblog_write_entry("CAN_InitializeXL");
	snprintf(szLog, MAX_LOG, "Channel: 0x%02X, BitrateXL: {%s}", Channel, BitrateXL);
	pcblog_write_param("CAN_InitializeXL", szLog);
	/* forward call */
	sts = pcanbasic_initialize_xl(Channel, BitrateXL);
	pcblog_write_exit("CAN_InitializeXL", sts);
	PCB_MUTEX_UNLOCK(&g_mut);
	return sts;
}
...
LINUX_API_VISIBILITY TPCANStatus CAN_ReadXL(
		TPCANHandle Channel,
	...
	PCB_MUTEX_LOCK(&g_mut);
	/* logging */
	pcblog_write_entry("CAN_ReadXL");
	snprintf(szLog, MAX_LOG,
			 "Channel: 0x%02X, MessageBuffer: 0x%p, TimestampBuffer: 0x%p",
			 Channel, (void *)MessageBuffer, (void *)TimestampBuffer);
	pcblog_write_param("CAN_ReadXL", szLog);
	/* forward call */
	sts = pcanbasic_read_xl(Channel, MessageBuffer, TimestampBuffer);

	if (sts == PCAN_ERROR_OK)
		pcblog_write_canxl_msg(Channel, LOG_FUNCTION_READ, MessageBuffer);
	pcblog_write_exit("CAN_ReadXL", sts);
	PCB_MUTEX_UNLOCK(&g_mut);
	return sts;
}
...
LINUX_API_VISIBILITY TPCANStatus CAN_WriteXL(
		TPCANHandle Channel,
		TPCANMsgXL* MessageBuffer) {
	TPCANStatus sts;
	char szLog[MAX_LOG];
	...
	PCB_MUTEX_LOCK(&g_mut);
	/* logging */
	pcblog_write_entry("CAN_WriteXL");
	snprintf(szLog, MAX_LOG, "Channel: 0x%02X, MessageBuffer: 0x%p", Channel,
			 (void *)MessageBuffer);
	pcblog_write_param("CAN_WriteXL", szLog);
	/* forward call */
	sts = pcanbasic_write_xl(Channel, MessageBuffer);

	if (sts == PCAN_ERROR_OK)
		pcblog_write_canxl_msg(Channel, LOG_FUNCTION_WRITE, MessageBuffer);
	pcblog_write_exit("CAN_WriteXL", sts);
	PCB_MUTEX_UNLOCK(&g_mut);
	return sts;
}
