...
static const char* pcbtrace_get_msgtype(__u32 msgtype);
static const char* pcbtrace_get_canid(__u32 canid, __u32 msgtype, char* buf, size_t size, int *has_no_canid);
...
int pcbtrace_write_header(struct pcbtrace_ctx *ctx, enum pcbtrace_version version) {
	...
	default:
		...
		break;
	case V3_0:
		snprintf(buf, PCBTRACE_MAX_MSG, ";$FILEVERSION=3.0\n");
		break;
	}
	...
	case V3_0:
		// see flags info in https://www.peak-system.com/produktcd/Pdf/English/PEAK_CAN_TRC_File_Format.pdf
		if (ctx->flags & TRACE_FILE_DATA_LENGTH)
			snprintf(buf, PCBTRACE_MAX_MSG, ";$COLUMNS=N,O,T,I,d,V,S,A,r,s,l,D\n");
		else
			snprintf(buf, PCBTRACE_MAX_MSG, ";$COLUMNS=N,O,T,I,d,V,S,A,r,s,L,D\n");
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
	...
	if (version == V3_0) {		
		...
		if (n <= 0)
			return -errno;
		snprintf(buf, PCBTRACE_MAX_MSG, ";   Index      Time    Type           VCID    AF     RRS      Data ... \n");
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		snprintf(buf, PCBTRACE_MAX_MSG, ";   |          Offset  |  CAN-ID/  Rx/|  SDT  |      | SEC    | \n");
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		snprintf(buf, PCBTRACE_MAX_MSG, ";   |          [ms]    |  PID      Tx |  |    |      | | DL");
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		if (ctx->flags & TRACE_FILE_DATA_LENGTH)
			...
			snprintf(buf, PCBTRACE_MAX_MSG, "C  | \n");
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		snprintf(buf, PCBTRACE_MAX_MSG, ";   |          |       |  |        |  |  |    |      | | |    | \n");
...
int pcbtrace_open_next(struct pcbtrace_ctx *ctx) {
	...
	if ((ctx->flags & TRACE_FILE_OVERWRITE) || (access(ctx->filename, F_OK) != 0)) {
		...
		if (ctx->pfile)
			return pcbtrace_write_header(ctx, ctx->version);
		return -errno;
	}
	else {
		return -EOPNOTSUPP;
...
const char* pcbtrace_get_msgtype(__u32 msgtype) {
	char* result;
	__u8 is_frame = 0;

	/* check echo frame first (combination of ERRFRAME and STATUS) */
	if ((msgtype & PCAN_MESSAGE_ECHO) == PCAN_MESSAGE_ECHO) {
		is_frame = 1;
	}
	/* check error frame */
	else if ((msgtype & PCAN_MESSAGE_ERRFRAME) == PCAN_MESSAGE_ERRFRAME) {
		result = "ER";
	}	
	/* check status frame */
	else if ((msgtype & PCAN_MESSAGE_STATUS) == PCAN_MESSAGE_STATUS) {
		result = "ST";
	}
	else {	/* msg is valid */
		is_frame = 1;
	...
	if (is_frame) {
		if ((msgtype & PCAN_MESSAGE_XL) == PCAN_MESSAGE_XL) {
			...
			/* check CAN FD */
		else if ((msgtype & PCAN_MESSAGE_FD) == PCAN_MESSAGE_FD) {
			/* check BRS & ESI */
			if ((msgtype & PCAN_MESSAGE_BRS) == PCAN_MESSAGE_BRS &&
				(msgtype & PCAN_MESSAGE_ESI) == PCAN_MESSAGE_ESI) {
				result = "BI";
			}
			/* check BRS */
			else if ((msgtype & PCAN_MESSAGE_BRS) == PCAN_MESSAGE_BRS) {
				result = "FB";
			}
			/* check ESI */
			else if ((msgtype & PCAN_MESSAGE_ESI) == PCAN_MESSAGE_ESI) {
				result = "FE";
			}
			else {
...
const char* pcbtrace_get_canid(__u32 canid, __u32 msgtype, char* buf, size_t size, int *has_no_canid) {
	char *ptr = buf;
	
	*has_no_canid = ((msgtype & PCAN_MESSAGE_ECHO) != PCAN_MESSAGE_ECHO);
	*has_no_canid &= (
		(msgtype & PCAN_MESSAGE_ERRFRAME) == PCAN_MESSAGE_ERRFRAME ||
		(msgtype & PCAN_MESSAGE_STATUS) == PCAN_MESSAGE_STATUS ||
		(msgtype & PCAN_MESSAGE_ERROR_NOTIFICATION) == PCAN_MESSAGE_ERROR_NOTIFICATION ||
		(msgtype & PCAN_MESSAGE_PROTOCOL_EXCEPTION) == PCAN_MESSAGE_PROTOCOL_EXCEPTION
		);
	...
	if (*has_no_canid) {
		...
		if ((msgtype & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
			snprintf(buf, size, "%08x", canid);
		else
			snprintf(buf, size, "    %04x", canid);
...
int pcbtrace_write_msg(struct pcbtrace_ctx *ctx, TPCANMsgFD *msg, int data_len, struct timeval *tv, int rx) {
	...
	len += n;
	/* write: "Type " */
	snprintf(buf, PCBTRACE_MAX_MSG, "%s ", pcbtrace_get_msgtype(msg->MSGTYPE));
	n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
	if (n <= 0)
		return -errno;
	len += n;
	/* write: "ID" */
	pcbtrace_get_canid(msg->ID, msg->MSGTYPE, buf, PCBTRACE_MAX_CHAR_SIZE, &has_no_canid);
	n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
	if (n <= 0)
		return -errno;
	/* write: " Rx/Tx " */
...
int pcbtrace_write_msg_xl(struct pcbtrace_ctx *ctx, TPCANMsgXL *msg, int data_len, struct timeval *tv, int rx) {
	char buf[PCBTRACE_MAX_MSG];
	int i, n, len;
	int has_no_canid, buf_init;
#if DEPRECATED_VERSION_1_1
	...
	/* write: "Frame_Nb   Time_msec.micros" */
	int64_t ts = (tv->tv_sec - ctx->time_start.tv_sec) * 1000 + (tv->tv_usec / 1000);
	if (ts < 0) {
		pcanlog_log(LVL_INFO, "Tracing a message that has a timestamp before channel's initialisation: TS=%ld < Start=%ld.\n", tv->tv_sec, ctx->time_start.tv_sec);
	}
	snprintf(buf, PCBTRACE_MAX_MSG, "%8lu %9" PRId64 ".%03lu ",
		ctx->msg_cnt, ts, (tv->tv_usec % 1000));
	n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
	if (n <= 0)
		return -errno;
	...
	len += n;
	/* write: "PID" */
	pcbtrace_get_canid(msg->PID, msg->MSGTYPE, buf, PCBTRACE_MAX_CHAR_SIZE, &has_no_canid);
	n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
	if (n <= 0)
		return -errno;
	/* write: " Rx/Tx " */
	...
	if (n <= 0)
		return -errno;
	/* write: XL info */
	if ((msg->MSGTYPE & PCAN_MESSAGE_XL) == PCAN_MESSAGE_XL) {
		/* write: "VCID " */
		snprintf(buf, PCBTRACE_MAX_MSG, "%02X ", msg->VCID);
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		/* write: "SDT " */
		snprintf(buf, PCBTRACE_MAX_MSG, "%02X ", msg->SDT);
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		/* write: "AF " */
		snprintf(buf, PCBTRACE_MAX_MSG, "%08X ", msg->AF);
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		/* write: "RRS " */
		snprintf(buf, PCBTRACE_MAX_MSG, "%X ", msg->RRS);
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
		/* write: "SEC " */
		snprintf(buf, PCBTRACE_MAX_MSG, "%X ", msg->SEC);
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
	}
	else {
		/* write: empty fields for "VCID SDT AF RRS SEC" */
		snprintf(buf, PCBTRACE_MAX_MSG, "-  -  -        - - ");
		n = fwrite(buf, strlen(buf), sizeof(char), ctx->pfile);
		if (n <= 0)
			return -errno;
	...
	if ((msg->MSGTYPE & PCAN_MESSAGE_ECHO) == PCAN_MESSAGE_ECHO) {
		...
		buf[0] = 0;
		/* force data: data[0..3]=msg[0..3] */
		snprintf(bufData, 10, "%.2X ", (uint8_t)msg->PID);		
		for (i = 0; i < data_len; i++)
		{
			snprintf(bufData, 10, "%.2X ", (i < msg->DLC) ?  msg->DATA[i] : 0);
			strncat(buf, bufData, PCBTRACE_MAX_MSG - strlen(buf));
