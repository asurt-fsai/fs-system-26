...
#define MAX_LOG		256
#define MAX_LOG_XL	MAX_LOG + (2048 * 3)
...
int pcblog_write_canxl_msg(TPCANHandle channel, int direction, TPCANMsgXL *pmsg) {
	...
	if (g_pcblog.enabled && (g_pcblog.flags & direction) && pmsg) {
		char szId[50];
		char szFormat[MAX_LOG];
		char szMessage[2 * MAX_LOG + MAX_LOG_XL];
		int i;
		int len = pcanbasic_get_xl_len(pmsg->DLC);
		
		/* format CAN ID */
		sprintf(szId, "0x%08X", pmsg->PID);
		/* format MSGTYPE */
		sprintf(szFormat, "[\
			VCID=0x%02X|\
			MSGTYPE=0x%02X|\
		...
		sprintf(
				szMessage,
				"    CHANNEL    0x%02X (%s) %s PID=%s DLC=%d Len=%u, Data=",
				(unsigned int) channel, 
				direction == LOG_FUNCTION_WRITE ? "OUT" : "IN", 
				szFormat, szId, pmsg->DLC, len);
