...
/*
 * struct pcanfd_msg *pcanfd_from_msg(struct pcanfd_msg *pf,
 *				      const TPCANRdMsg *msg)
 *
 *	Convert old TPCANRdMsg to new struct pcanfd_msg representation.
 */
struct pcanfd_msg *pcanfd_from_msg(struct pcanfd_msg *pf, const TPCANRdMsg *msg)
{
	pf->type = (msg->Msg.MSGTYPE & MSGTYPE_STATUS) ?
				PCANFD_TYPE_STATUS : PCANFD_TYPE_CAN20_MSG;
	pf->id = msg->Msg.ID;
	pf->flags = msg->Msg.MSGTYPE & ~MSGTYPE_STATUS;
	pf->data_len = (msg->Msg.LEN > PCANFD_CAN20_MAXDATALEN) ?
				PCANFD_CAN20_MAXDATALEN : msg->Msg.LEN;
...
{
#ifdef DEBUG
	__fprintf(stddbg, "%s(fd=%d flags=%lxh)\n", __func__, fd, flags);
#endif
