...
static int get_xl_len(__u16 dlc, int type) {
	if ((type & PCAN_MESSAGE_XL) == PCAN_MESSAGE_XL) {
		return (dlc & 0x7FF) + 1;
	}
	return get_fd_len(dlc);
}
////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	TPCANMsgXL Message;
	TPCANTimestampXL ts_app_start;
	TPCANTimestampXL ts;
	TPCANTimestampXL ts_prev;
	TPCANTimestampXL ts_diff;
	TPCANStatus Status;
	...
	if (!mainr)
	{
		printf("pcanreadxl(%xh): unable to setup main RT task\n",
			   PCAN_DEVICE);
		return -1;
	}
	...
	while (g_app_stop == 0)
	{
		while ((Status = CAN_ReadXL(pcan_device, &Message, &ts)) == PCAN_ERROR_QRCVEMPTY && g_app_stop == 0)
			if (usleep(100))
				break;

		if (Status != PCAN_ERROR_OK)
		{
			printf("CAN_ReadXL(%xh) failure 0x%x\n", pcan_device, (int)Status);
			break;
		}
		else
		{
		...
		if ((Message.MSGTYPE & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
			printf("  - R ID:%08X ", (int)Message.PID);
		else
			printf("  - R ID:    %04X ", (int)Message.PID);

		printf("DLC:%1X TYPE:%02X ", (int)Message.DLC, (int)Message.MSGTYPE);
		
		if ((Message.MSGTYPE & PCAN_MESSAGE_XL) == PCAN_MESSAGE_XL) {
			printf("VCID:%02X SDT:%02X ", (int)Message.VCID, (int)Message.SDT);
			printf("AF:%08X RRS:%1X SEC:%1X ", (int)Message.AF, (int)Message.RRS, (int)Message.SEC);
		}
		...
		printf("DATA:");
		for (int i = 0; i < get_xl_len(Message.DLC, Message.MSGTYPE); ++i)
