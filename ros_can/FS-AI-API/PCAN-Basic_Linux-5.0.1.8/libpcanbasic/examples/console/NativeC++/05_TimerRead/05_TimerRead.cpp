...
TPCANStatus TimerRead::ReadMessageXL()
{
    TPCANMsgXL CANMsg;
...
void TimerRead::ProcessMessageCanXL(TPCANMsgXL msg, TPCANTimestampXL itsTimeStamp)
{
...
void TimerRead::FormatChannelName(TPCANHandle handle, LPSTR buffer, CANProtocol protocol)
{
    BYTE byChannel;
...
std::string TimerRead::GetMsgTypeStringXL(TPCANMessageTypeXL msgType, bool rrsFlag, bool secFlag)
{
    std::string strTemp;
...
std::string TimerRead::GetIdString(UINT32 id, TPCANMessageTypeXL msgType) 
{
    char result[MAX_PATH] = { 0 };
    if ((msgType & PCAN_MESSAGE_EXTENDED) == PCAN_MESSAGE_EXTENDED)
    {
...
int TimerRead::GetLengthFromDLC(WORD dlc)
{
    if (dlc < 0)
        return 1;
...
std::string TimerRead::GetDataString(BYTE data[], TPCANMessageTypeXL msgType, int dataLength)
{
    if ((msgType & PCAN_MESSAGE_RTR) == PCAN_MESSAGE_RTR)
        return "Remote Request";
    else
...
std::string TimerRead::GetDataString(BYTE  data[], int dataLength)
{
