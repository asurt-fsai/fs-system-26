...
//3ayza ageeb el wheel speed (unit rpm) men el car speed 3ashan adrabha fel belt ratio w ahseb el motor speed 3ashan heya deeh el ha set it w heya deeeh el function mestaneyaha 
//considering i have belt ratio 3.5:1 then motor speed = wheel speed *3.5
void CanInterface::commandCallback(ackermann_msgs::msg::AckermannDriveStamped::SharedPtr msg) {
  if (driving_flag_) {
    // check speed instead of acceleration, calculate rpm instead of torque
    // set torque to MAX_TORQUE 
    ...
    torque_ = MAX_TORQUE_;  // Set to maximum torque
    rpm_request_ = 3.5*((speed *60) / (2* M_PI * WHEEL_RADIUS_));  // Convert speed (we want it to be m/minute ) to get wheel rpm then multiplied with 3.5 to convert it to motor speed rpm

    // ana mesh bal3ab fe acceleration so i dont need it
    // if (acceleration > 0.0) {
    //   braking_ = 0.0;
