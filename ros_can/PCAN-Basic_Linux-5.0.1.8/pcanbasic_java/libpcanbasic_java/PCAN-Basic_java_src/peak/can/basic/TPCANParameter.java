...
public enum TPCANParameter {
    ...
    /**
     * Value assigned to a 32 digital I/O pins of a PCAN-USB Chip - Multiple
     * digital I/O pins to 1 = High
     */
     ...
     */
    PCAN_DEVICE_PART_NUMBER(0x2D),
    /**
     * Activation status of hard reset processing via CAN_Reset calls
     */
    PCAN_HARD_RESET_STATUS(0x2E),
    /**
     * Communication direction of a PCAN-Channel representing a PCAN-LAN
     * interface
     */
    PCAN_LAN_CHANNEL_DIRECTION(0x2F),
    /**
     * Get the global unique device identifier (GUID) associated to a device
     */
    PCAN_DEVICE_GUID(0x30),
    /**
     * Configured bit rate as TPCANBitrateCC value
     */
    PCAN_BITRATE_INFO_CC(0x31),
    /**
     * Configured bit rate as TPCANBitrateXL string
     */
    PCAN_BITRATE_INFO_XL(0x32),
    /**
     * Configured CAN XL Bus speed as Bits per seconds
     */
    PCAN_BUSSPEED_XL(0x33),
    /**
     * @deprecated Use PCAN_BUSSPEED_FD instead
     */
    PCAN_BUSSPEED_DATA(PCAN_BUSSPEED_FD.value),
    /**
     * @deprecated Use PCAN_BITRATE_INFO_BTR instead
     */
    PCAN_BITRATE_INFO(PCAN_BITRATE_INFO_BTR.value);
    ...
    public static TPCANParameter[] customizableParameters() {
        ...
        result.add(TPCANParameter.PCAN_ALLOW_ECHO_FRAMES);
        result.add(TPCANParameter.PCAN_DEVICE_PART_NUMBER);
        result.add(TPCANParameter.PCAN_HARD_RESET_STATUS);
        result.add(TPCANParameter.PCAN_LAN_CHANNEL_DIRECTION);
        result.add(TPCANParameter.PCAN_DEVICE_GUID);
        result.add(TPCANParameter.PCAN_BITRATE_INFO_CC);
        result.add(TPCANParameter.PCAN_BITRATE_INFO_XL);
        result.add(TPCANParameter.PCAN_BUSSPEED_XL);
    ...
    public static String getParameterDescription(TPCANParameter param) {
        ...
        switch (param) {
            ...
            case PCAN_BITRATE_INFO_BTR:
                description = "This Parameter is used to read the currently configured BTR0/BTR1 communication speed of a PCAN Channel connected as standard CAN.";
                break;
            ...
            case PCAN_ALLOW_ECHO_FRAMES:
                description = "This Parameter is used to configure the reception of messages of type \"PCAN_MESSAGE_ECHO\" (self received messages) within a PCAN-Channel.";
                break;
            case PCAN_DEVICE_PART_NUMBER:
                description = "This parameter is used to get the part number of the device represented by the PCAN-Channel.";
                break;
            case PCAN_HARD_RESET_STATUS:
                description = "This parameter is used to configure the status of hard reset processing via CAN_Reset calls.";
                break;
            case PCAN_LAN_CHANNEL_DIRECTION:
                description = "This parameter is used to get the communication flow used by a PCAN-LAN device associated with a PCAN-Channel.";
                break;
            case PCAN_DEVICE_GUID:
                description = "This parameter is used to get the global unique device identifier (GUID) associated to a device.";
                break;
            case PCAN_BITRATE_INFO_CC:
                description = "This parameter is used to get the configured bit rate as TPCANBitrateCC value.";
                break;
            case PCAN_BITRATE_INFO_XL:
                description = "This parameter is used to get the configured bit rate as TPCANBitrateXL string.";
                break;
            case PCAN_BUSSPEED_XL:
                description = "This parameter is used to get the configured CAN XL Bus speed as Bits per seconds.";
                break;
            default:
                break;
        }
