...
#define TYPE_SIGNATURE_DOUBLE		"()D"
...
#define printf_dbg(...)	fprintf(stdout, __VA_ARGS__);fflush(stdout)
#if _DEBUG_VERBOSE == 1
#define printf_dbg_verbose(...)	fprintf(stdout, __VA_ARGS__)
#else
#define printf_dbg_verbose(...)
#endif
...
int GetClassEnumValue(JNIEnv *env, jobject* target, const char *className, const char *valueName, const char *byteCodeTypeName)
{
	...
	if (valueName == NULL)
		valueName = "";
	printf_dbg_verbose("Calling %s(className='%s', valueName='%s', byteCodeTypeName='%s')...\n", __FUNCTION__, className, valueName, byteCodeTypeName);
	// Find JClass
...
/// <returns>"A TPCANStatus error code"</returns>
JNIEXPORT jobject JNICALL Java_peak_can_basic_PCANBasic_InitializeXL
(JNIEnv* env, jobject obj, jobject Channel, jobject BitrateXL)
{
	TPCANStatus status;
	jobject jStatus;
	TPCANHandle pChannel;
	TPCANBitrateXL pBitrate;

	// prevent access to NULL function pointer with non XL version
	if (bIsXlCapable == false) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_InitializeXL: PCANBasic version without CAN XL support.");
		ParseTPCANStatusToJava(env, _LEGACY_PCAN_ERROR_ILLOPERATION, &jStatus);
		return jStatus;
	}
	// Parse Channel
	pChannel = (TPCANHandle)ParseEnumValueFromJava(env, &Channel, TYPE_SIGNATURE_SHORT);
	// Parse BitrateXL
	pBitrate = (TPCANBitrateXL)ParseEnumStrValueFromJava(env, &BitrateXL);
	//Call PCANBasic Function
	status = PCBasic_InitializeXl(pChannel, pBitrate);
	free(pBitrate);
	//Parse Result
	ParseTPCANStatusToJava(env, status, &jStatus);
...
(JNIEnv * env, jobject obj, jobject Channel, jobject MessageBuffer, jobject TimestampBuffer)
{
	...
	if (status == PCAN_ERROR_OK) {
		...
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadFD function: GetMethodID 'setType' for TPCANMsgFD failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		...
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadFD function: GetMethodID 'setDlc' for TPCANMsgFD failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		...
		len = GetLengthFromDlcFd((mymsg).DLC);
		...
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadFD function: GetMethodID 'setData' for TPCANMsgFD failed.");
			(*env)->DeleteLocalRef(env, byteArray);
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
...
/// should be passed as NULL"</param>
/// <returns>"A TPCANStatus error code"</returns>
JNIEXPORT jobject JNICALL Java_peak_can_basic_PCANBasic_ReadXL
(JNIEnv* env, jobject obj, jobject Channel, jobject MessageBuffer, jobject TimestampBuffer)
{
	TPCANHandle pChannel;
	TPCANMsgXL mymsg = { 0 };
	TPCANTimestampXL myRcvTime = { 0 };
	TPCANStatus status;
	jobject jStatus;
	jclass cls;
	jmethodID mid;
	...
	// prevent access to NULL function pointer with non XL version
	if (bIsXlCapable == false) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL: PCANBasic version without CAN XL support.");
		ParseTPCANStatusToJava(env, _LEGACY_PCAN_ERROR_ILLOPERATION, &jStatus);
		return jStatus;
	}
	// Verify MessageBuffer is not NULL
	if (MessageBuffer == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: MessageBuffer is null.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		return jStatus;
	}
	// Parse Channel
	...
	// Call CAN-API function
	if (TimestampBuffer != NULL)
		status = PCBasic_ReadXl(pChannel, &mymsg, &myRcvTime);
	else
		status = PCBasic_ReadXl(pChannel, &mymsg, NULL);
	// Copy message on success
	if (status == PCAN_ERROR_OK) {
		printf_dbg("ReadXL: [PID=0x%04X; TYPE=0x%04X; DLC=%d; SDT=0x%02X; VCID=0x%02X; AF=0x%08X; SEC=0x%02X; RRS=0x%02X]\n",
			mymsg.PID, mymsg.MSGTYPE, mymsg.DLC, mymsg.SDT, mymsg.VCID, mymsg.AF, mymsg.SEC, mymsg.RRS);
		if (TimestampBuffer != NULL) {
			// Update Java rcvTime variable
			cls = (*env)->GetObjectClass(env, TimestampBuffer);
			if (cls == NULL)
			{
				ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetObjectClass for TPCANTimestampXL failed.");
				ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
				(*env)->DeleteLocalRef(env, cls);
				return jStatus;
			}
			...
			if (mid == NULL)
			{
				ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setValue' for TPCANTimestampXL failed.");
				ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
				(*env)->DeleteLocalRef(env, cls);
				return jStatus;
			}
		...
		cls = (*env)->GetObjectClass(env, MessageBuffer);
		if (cls == NULL) {
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetObjectClass for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		// Set PID
		mid = (*env)->GetMethodID(env, cls, "setPID", "(I)V");
		if (mid == NULL) {
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setPID' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jint)(mymsg).PID);
		// Set VCID
		mid = (*env)->GetMethodID(env, cls, "setVCID", "(B)V");
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setVCID' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jbyte)(mymsg).VCID);
		// Set MSGTYPE
		mid = (*env)->GetMethodID(env, cls, "setType", "(S)V");
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setType' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jshort)(mymsg).MSGTYPE);
		// Set DLC
		mid = (*env)->GetMethodID(env, cls, "setDlc", "(S)V");
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setDlc' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jshort)(mymsg).DLC);
		// Set SDT
		mid = (*env)->GetMethodID(env, cls, "setSDT", "(B)V");
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setSDT' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jbyte)(mymsg).SDT);
		// Set AF
		mid = (*env)->GetMethodID(env, cls, "setAF", "(I)V");
		if (mid == NULL) {
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setAF' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jint)(mymsg).AF);
		// Set RRS
		mid = (*env)->GetMethodID(env, cls, "setRRS", "(B)V");
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setRRS' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jbyte)(mymsg).RRS);
		// Set SEC
		mid = (*env)->GetMethodID(env, cls, "setSEC", "(B)V");
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setSEC' for TPCANMsgXL failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, (jbyte)(mymsg).SEC);

		// get actual length from DLC to prepare Data
		len = GetLengthFromDlc(mymsg.MSGTYPE, mymsg.DLC);
		// Create java byte array to set Data
		byteArray = (*env)->NewByteArray(env, len);
		if (byteArray == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: NewByteArray failed.");
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->SetByteArrayRegion(env, byteArray, 0, len, (jbyte*)(mymsg).DATA);
		// Set Data
		mid = (*env)->GetMethodID(env, cls, "setData", "([BS)V");
		if (mid == NULL)
		{
			ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_ReadXL function: GetMethodID 'setData' for TPCANMsgXL failed.");
			(*env)->DeleteLocalRef(env, byteArray);
			ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
			(*env)->DeleteLocalRef(env, cls);
			return jStatus;
		}
		(*env)->CallVoidMethod(env, MessageBuffer, mid, byteArray, len);
		(*env)->DeleteLocalRef(env, byteArray);
		(*env)->DeleteLocalRef(env, cls);
	}
	//Parse Result
...
(JNIEnv *env, jobject obj, jobject Channel, jobject Parameter, jobject Buffer, jint BufferLength)
{
	...
	// Check Buffer's type: PEAK_MUTABLE_INTEGER
	if (!found) {
	...
	// Check Buffer's type: STRINGBUFFER
	if (!found) {
		...
		if ((*env)->IsInstanceOf(env, Buffer, cls) == JNI_TRUE)
		{
			...
			if (status == PCAN_ERROR_OK) {
				...
				if (pParameter == PCAN_API_VERSION && strncmp(charBuffer, STRFILEVER, strlen(STRFILEVER)) != 0) {
					appendTextToJavaStringBuffer(env, Buffer, " (JNI=" STRFILEVER ")");
				}
			}
		}
		(*env)->DeleteLocalRef(env, cls);
	}
	// Check Buffer's type: PEAK_MUTABLE_LONG
	if (!found) {
		...
		if ((*env)->IsInstanceOf(env, Buffer, cls) == JNI_TRUE)
		{
			...
			unsigned long long longBuffer = 0;
			//Call PCANBasic Function
			status = PCBasic_GetParameter(pChannel, pParameter, &longBuffer, sizeof(longBuffer));
			printf_dbg("%s - parameter is %s: (%llX)...\n", __FUNCTION__, className, longBuffer);
			// Set result into object
			mid = (*env)->GetMethodID(env, cls, "setValue", "(J)V");
	...
	// Check Buffer's type: PEAK_TPCANChannelInformation_ARRAY
	if (!found) {
...
(JNIEnv *env, jobject obj, jobject Channel, jobject Parameter, jobject Buffer, jint BufferLength)
{
	...
	// Check Buffer Is Instance Of MutableInteger
	if (!found) {
		className = JAVA_CLASS_PEAK_MUTABLE_INTEGER;
		cls = (*env)->FindClass(env, className);
		if (cls == NULL) {
	...
	// Check Buffer Is Instance Of MutableLong
	if (!found) {
		className = JAVA_CLASS_PEAK_MUTABLE_LONG;
		cls = (*env)->FindClass(env, className);
		if (cls == NULL) {
		...
		if ((*env)->IsInstanceOf(env, Buffer, cls) == JNI_TRUE) {
			printf_dbg_ts();
			printf_dbg("%s - parameter is %s...\n", __FUNCTION__, className);
			mid = (*env)->GetMethodID(env, cls, "getValue", TYPE_SIGNATURE_LONG);
			if (mid != NULL) {
				found = 1;
				longBuffer = (unsigned long long)(*env)->CallLongMethod(env, Buffer, mid);
				printf_dbg("%s - parameter is %s: (%llX)...\n", __FUNCTION__, className, longBuffer);
				//Call PCANBasic Function
				status = PCBasic_SetParameter(pChannel, pParameter, &longBuffer, sizeof(longBuffer));
			}
		}
		(*env)->DeleteLocalRef(env, cls);
	}
	// Check Buffer Is Instance Of INTEGER
	if (!found) {
		...
		if (mid != NULL) {
			found = 1;
			printf_dbg_ts();
			printf_dbg("%s - parameter has a method: int getValue()...\n", __FUNCTION__);
			...
			// object does not have a getValue() prototype: ignore exception
...
(JNIEnv *env, jobject obj, jobject Channel, jobject MessageBuffer)
{
	...
	cls = (*env)->GetObjectClass(env, MessageBuffer);
	if (cls == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteFD function: GetObjectClass for TPCANMsgFD failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		return jStatus;
	}
	//Gets Method ID
	mid = (*env)->GetMethodID(env, cls, "getID", TYPE_SIGNATURE_INTEGER);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteFD function: GetMethodID('getID') for TPCANMsgFD failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	...
	mid = (*env)->GetMethodID(env, cls, "getType", TYPE_SIGNATURE_BYTE);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteFD function: GetMethodID('getType') for TPCANMsgFD failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	...
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteFD function: GetMethodID('getLength') for TPCANMsgFD failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	...
	mid = (*env)->GetMethodID(env, cls, "getData", TYPE_SIGNATURE_ARRAY_BYTE);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteFD function: GetMethodID('getData') for TPCANMsgFD failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	...
	len = GetLengthFromDlcFd(message.DLC);
	for (i = 0; i < (len); i++) {
		(message.DATA)[i] = nativeByteArray[i];
	}
	// Calls PCANBasic Function
...
/// <returns></returns>
JNIEXPORT jobject JNICALL Java_peak_can_basic_PCANBasic_WriteXL
(JNIEnv* env, jobject obj, jobject Channel, jobject MessageBuffer)
{
	TPCANStatus status;
	jobject jStatus;
	TPCANHandle pChannel;
	TPCANMsgXL message = { 0 };
	int i;
	jclass cls;
	jmethodID mid;
	jbyteArray javaByteArray;
	...
	// prevent access to NULL function pointer with non XL version
	if (bIsXlCapable == false) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL: PCANBasic version without CAN XL support.");
		ParseTPCANStatusToJava(env, _LEGACY_PCAN_ERROR_ILLOPERATION, &jStatus);
		return jStatus;
	}
	// Parse Channel
	...
	// Verify MessageBuffer is not NULL
	if (MessageBuffer == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: MessageBuffer is null.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		return jStatus;
	}
	//Gets class
	cls = (*env)->GetObjectClass(env, MessageBuffer);
	if (cls == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetObjectClass failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		return jStatus;
	}

	// Get Method PID
	mid = (*env)->GetMethodID(env, cls, "getPID", TYPE_SIGNATURE_INTEGER);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getPID') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.PID = (*env)->CallIntMethod(env, MessageBuffer, mid);
	// Get Method VCID
	mid = (*env)->GetMethodID(env, cls, "getVCID", TYPE_SIGNATURE_BYTE);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getVCID') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.VCID = (*env)->CallByteMethod(env, MessageBuffer, mid);
	// Get Method MSGTYPE
	mid = (*env)->GetMethodID(env, cls, "getType", TYPE_SIGNATURE_SHORT);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getType') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.MSGTYPE = (*env)->CallShortMethod(env, MessageBuffer, mid);
	// Get Method DLC
	mid = (*env)->GetMethodID(env, cls, "getDlc", TYPE_SIGNATURE_SHORT);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getDlc') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.DLC = (*env)->CallShortMethod(env, MessageBuffer, mid);
	// Get Method SDT
	mid = (*env)->GetMethodID(env, cls, "getSDT", TYPE_SIGNATURE_BYTE);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getSDT') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.SDT = (*env)->CallByteMethod(env, MessageBuffer, mid);
	// Get Method AF
	mid = (*env)->GetMethodID(env, cls, "getAF", TYPE_SIGNATURE_INTEGER);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getAF') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.AF = (*env)->CallIntMethod(env, MessageBuffer, mid);
	// Get Method RRS
	mid = (*env)->GetMethodID(env, cls, "getRRS", TYPE_SIGNATURE_BYTE);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getRRS') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.RRS = (*env)->CallByteMethod(env, MessageBuffer, mid);
	// Get Method SEC
	mid = (*env)->GetMethodID(env, cls, "getSEC", TYPE_SIGNATURE_BYTE);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getSEC') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	message.SEC = (*env)->CallByteMethod(env, MessageBuffer, mid);

	//get Data
	mid = (*env)->GetMethodID(env, cls, "getData", TYPE_SIGNATURE_ARRAY_BYTE);
	if (mid == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetMethodID('getData') failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	javaByteArray = (jbyteArray)(*env)->CallObjectMethod(env, MessageBuffer, mid);
	if (javaByteArray == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: CallObjectMethod failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	...
	nativeByteArray = (*env)->GetByteArrayElements(env, javaByteArray, NULL);
	if (nativeByteArray == NULL) {
		ThrowExByName(env, JAVA_CLASS_JRE_NULL_POINTER_EXCEPTION, "Java_peak_can_basic_PCANBasic_WriteXL function: GetByteArrayElements failed.");
		ParseTPCANStatusToJava(env, PCAN_ERROR_UNKNOWN, &jStatus);
		(*env)->DeleteLocalRef(env, cls);
		return jStatus;
	}
	// get actual length from DLC
	len = GetLengthFromDlc(message.MSGTYPE, message.DLC);
	for (i = 0; i < (len); i++) {
		(message.DATA)[i] = nativeByteArray[i];
	}

	printf_dbg("WriteXL: [PID=0x%04X; TYPE=0x%04X; DLC=%d; SDT=0x%02X; VCID=0x%02X; AF=0x%08X; SEC=0x%02X; RRS=0x%02X]\n",
		message.PID, message.MSGTYPE, message.DLC, message.SDT, message.VCID, message.AF, message.SEC, message.RRS);
	// Calls PCANBasic Function
	status = PCBasic_WriteXl(pChannel, &message);
	// Parses Result
	ParseTPCANStatusToJava(env, status, &jStatus);
	(*env)->ReleaseByteArrayElements(env, javaByteArray, nativeByteArray, JNI_ABORT);
	(*env)->DeleteLocalRef(env, cls);
...
void ParseTPCANStatusToJava(JNIEnv *env, TPCANStatus source, jobject* target)
{
	...
	printf_dbg_verbose("Calling %s(source='0x%02X'): constantName='%s'...\n", __FUNCTION__, source, constantName);
...
{
	return (dlc <= 0x08) ? dlc : 8; // buffer overflow protection
}
...
jbyte GetLengthFromDlcFd(BYTE dlc)
{
	...
	case 15:
		return 64;
	default:
		return (dlc <= 0x0F) ? dlc : 64; // buffer overflow protection
	}
}
...
{
	return (dlc < 2048) ? dlc + 1 : 2048; // buffer overflow protection
}

jshort GetLengthFromDlc(WORD type, WORD dlc)
{
	if ((type & PCAN_MESSAGE_XL) == PCAN_MESSAGE_XL)
		return GetLengthFromDlcXl(dlc);
	else if ((type & PCAN_MESSAGE_FD) == PCAN_MESSAGE_FD)
		return GetLengthFromDlcFd((BYTE)dlc);
	else
		return GetLengthFromDlcCc((BYTE)dlc);
}