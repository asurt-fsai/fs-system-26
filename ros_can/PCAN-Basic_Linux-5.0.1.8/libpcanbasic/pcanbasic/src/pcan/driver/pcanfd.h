...
#define __flex_array_of_struct(_n)				\
	_n##s_0 {						\
		__u32		count;				\
		struct _n	list[] __counted_by(count);	\
	}
...
#define PCANFD_TYPE_ERROR_MSG	4 /* Deprecated in v9 (see PCANXL_TYPE_ERROR) */
#define PCANFD_TYPE_ERROR	PCANFD_TYPE_ERROR_MSG
...
struct pcan_bittiming {
	...
	union {
		__u32	bitrate_real;	/* deprecated in v9 - use bitrate */
		__u32	ssp_offset;	/* CAN-XL devices only */
	};
};
...
#define PCANFD_FEATURE_ERR_GEN		0x00002000
...
/* PCANFD_OPT_ERR_GEN Error Generator */
#define PCANFD_ERR_GEN_STOP		0
#define PCANFD_ERR_GEN_START_SINGLE	1
#define PCANFD_ERR_GEN_START_PERIODIC	2

struct pcanfd_error_generator {
	__u16	mode;
	__u16	bit_pos;
	__u32	can_id;
	__u16	to_kill_nb;
