...
/**
 * @fn int pcblog_write_canxl_msg(TPCANHandle channel, int direction, TPCANMsgXL* pmsg)
 * @brief Formats and logs a CAN XL message from a PCANBasic channel.
 *
 * @param[in] channel a PCANBasic channel handle
 * @param[in] direction of the message: Rx (LOG_FUNCTION_READ) or Tx (LOG_FUNCTION_WRITE)
 * @param[in] pmsg buffer containing the CANXL message
 */
int pcblog_write_canxl_msg(TPCANHandle channel, int direction, TPCANMsgXL *pmsg);
