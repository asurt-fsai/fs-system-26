...
#include <pcanxl.h>		/* PCAN_ERROR_BUS + PCANXL_INIT_XL types */
...
#define PCAN_FILEINFO_DEVICE_GUID		"device_guid"
#define PCAN_FILEINFO_DEVID 			"devid"
#define PCAN_FILEINFO_DEV_NAME			"dev_name"
#define PCAN_FILEINFO_ERRORS 			"errors"
#define PCAN_FILEINFO_FD_BITRATE		"fd_bitrate"
#define PCAN_FILEINFO_FD_BRP			"fd_brp"
#define PCAN_FILEINFO_FD_SJW			"fd_sjw"
#define PCAN_FILEINFO_FD_SP				"fd_sp"
#define PCAN_FILEINFO_FD_SSP_OFFSET		"fd_ssp_offset"
#define PCAN_FILEINFO_FD_TQ				"fd_tq"
#define PCAN_FILEINFO_FD_TSEG1			"fd_tseg1"
#define PCAN_FILEINFO_FD_TSEG2			"fd_tseg2"
...
#define PCAN_FILEINFO_NOM_SP			"nom_sp"
...
#define PCAN_FILEINFO_RXERR_COUNTER		"rx_error_counter"
#define PCAN_FILEINFO_RX_ERRORS			"rx_errors"
#define PCAN_FILEINFO_RX_FIFO_RATIO		"rx_fifo_ratio"
#define PCAN_FILEINFO_RX_FRAMES_COUNTER	"rx_frames_counter"
#define PCAN_FILEINFO_RX_FRAMES			"rx_frames"
...
#define PCAN_FILEINFO_TXERR_COUNTER		"tx_error_counter"
#define PCAN_FILEINFO_TX_ERRORS			"tx_errors"
#define PCAN_FILEINFO_TX_FIFO_RATIO		"tx_fifo_ratio"
#define PCAN_FILEINFO_TX_FRAMES_COUNTER	"tx_frames_counter"
#define PCAN_FILEINFO_TX_FRAMES			"tx_frames"
...
#define PCAN_FILEINFO_XL_BITRATE 		"xl_bitrate"
#define PCAN_FILEINFO_XL_ES				"xl_es"
#define PCAN_FILEINFO_XL_PWM_LONG		"xl_pwml"
#define PCAN_FILEINFO_XL_PWM_OFFSET		"xl_pwmo"
#define PCAN_FILEINFO_XL_PWM_SHORT		"xl_pwms"
#define PCAN_FILEINFO_XL_SJW 			"xl_sjw"
#define PCAN_FILEINFO_XL_SP 			"xl_sp"
#define PCAN_FILEINFO_XL_SSP 			"xl_ssp_offset"
#define PCAN_FILEINFO_XL_TQ 			"xl_tq"
#define PCAN_FILEINFO_XL_TSEG1			"xl_tseg1"
#define PCAN_FILEINFO_XL_TSEG2			"xl_tseg2"
#define PCAN_FILEINFO_XL_TMS			"xl_tms"
	...
	else
	{
		...
		if (read != -1) {
			...
			if (strcmp(filename, PCAN_FILEINFO_ADAPTER_NAME) == 0) {
				len = MIN(sizeof(pci->adapter_name), (unsigned int)read);
				...
				len = MIN(sizeof(pci->adapter_version), (unsigned int)read);
				...
				len = MIN(sizeof(pci->adapter_partnum), (unsigned int)read);
				...
				pci->availflag |= PCANINFO_FLAG_CTRLNB;
					 ...
					 strcmp(filename, PCAN_FILEINFO_FD_BITRATE) == 0 ||
				...
				len = MIN(sizeof(pci->dev), (unsigned int)read);
				...
				len = MIN(sizeof(pci->dev_name), (unsigned int)read);
				...
				len = MIN(sizeof(pci->type), (unsigned int)read);
			...
			else if (strcmp(filename, PCAN_FILEINFO_RX_ERRORS) == 0 ||
					strcmp(filename, PCAN_FILEINFO_RXERR_COUNTER) == 0) {
				pci->rxerr = strtoul(line, NULL, 0);
				pci->availflag |= PCANINFO_FLAG_RXERR;
			}
			else if (strcmp(filename, PCAN_FILEINFO_TX_ERRORS) == 0 ||
					strcmp(filename, PCAN_FILEINFO_TXERR_COUNTER) == 0) {
			...
			/* handle extra flags with availflag_ex */
				...
				pci->availflag_ex |= PCANINFO_FLAG_EX_NOM_BRP;
					 ...
					 strcmp(filename, PCAN_FILEINFO_NOM_SP) == 0) {
				...
				pci->availflag_ex |= PCANINFO_FLAG_EX_NOM_TQ;
					 ...
					 strcmp(filename, PCAN_FILEINFO_FD_BRP) == 0) {
				...
				pci->availflag_ex |= PCANINFO_FLAG_EX_DATA_BRP;
					 ...
					 strcmp(filename, PCAN_FILEINFO_FD_SP) == 0) {
				pci->data_sample_point = strtoul(line, NULL, 0);
				pci->availflag_ex |= PCANINFO_FLAG_EX_DATA_SAMPLE_POINT;
			}
			else if (strcmp(filename, PCAN_FILEINFO_FD_SSP_OFFSET) == 0) {
				pci->fd_ssp_offset = strtoul(line, NULL, 0);
				pci->availflag_ex |= PCANINFO_FLAG_EX_FD_SSP_OFFSET;
			}
			else if (strcmp(filename, PCAN_FILEINFO_DATA_SJW) == 0 ||
					 strcmp(filename, PCAN_FILEINFO_FD_SJW) == 0) {
				...
				pci->availflag_ex |= PCANINFO_FLAG_EX_DATA_SJW;
					 ...
					 strcmp(filename, PCAN_FILEINFO_FD_TSEG1) == 0) {
				...
				pci->availflag_ex |= PCANINFO_FLAG_EX_DATA_TSEG1;
					 ...
					 strcmp(filename, PCAN_FILEINFO_FD_TSEG2) == 0) {
				...
				pci->availflag_ex |= PCANINFO_FLAG_EX_DATA_TSEG2;
					 ...
					 strcmp(filename, PCAN_FILEINFO_FD_TQ) == 0) {
			...
			else if (strcmp(filename, PCAN_FILEINFO_DEVICE_GUID) == 0) {
				len = MIN(sizeof(pci->device_guid), (unsigned int)read);
				strncpy(pci->device_guid, line, len);
				if (len >= 1 && pci->device_guid[len - 1] == '\n')
					pci->device_guid[len - 1] = 0;
				pci->device_guid[sizeof(pci->device_guid) - 1] = 0;
				pci->availflag_ex |= PCANINFO_FLAG_EX_DEVICE_GUID;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_BITRATE) == 0) {
				pci->xl_bitrate = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_BITRATE;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_ES) == 0) {
				pci->xl_es = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_ES;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_PWM_LONG) == 0) {
				pci->xl_pwm_long = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_PWM_LONG;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_PWM_OFFSET) == 0) {
				pci->xl_pwm_offset = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_PWM_OFFSET;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_PWM_SHORT) == 0) {
				pci->xl_pwm_short = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_PWM_SHORT;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_SJW) == 0) {
				pci->xl_sjw = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_SJW;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_SP) == 0) {
				pci->xl_sp = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_SP;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_SSP) == 0) {
				pci->xl_ssp_offset = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_SSP_OFFSET;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_TQ) == 0) {
				pci->xl_tq = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_TQ;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_TSEG1) == 0) {
				pci->xl_tseg1 = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_TSEG1;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_TSEG2) == 0) {
				pci->xl_tseg2 = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_TSEG2;
			}
			else if (strcmp(filename, PCAN_FILEINFO_XL_TMS) == 0) {
				pci->xl_tms = strtoul(line, NULL, 0);
				pci->availflag_xl |= PCANINFO_FLAG_XL_TMS;
			}
			else {
				if (pcanlog_should_write(LVL_DEBUG)) {
					...
					if (strcmp(filename, PCAN_FILEINFO_UEVENT) == 0 ||
						...
						strcmp(filename, PCAN_FILEINFO_RX_FRAMES) == 0 ||
						...
						strcmp(filename, PCAN_FILEINFO_TX_FRAMES) == 0 ||
...
void pcaninfo_output(struct pcaninfo * pci) {
	...
	if (pci->availflag_ex & PCANINFO_FLAG_EX_DEVICE_GUID) {
		fprintf(stdout, "  \t- %s: \"%s\"\n", PCAN_FILEINFO_DEVICE_GUID, pci->device_guid);
		separator++;
	}
	...
	if (pci->availflag_xl & PCANINFO_FLAG_XL_BITRATE) {
		fprintf(stdout, "  \t- %s: %sBit/s\n", PCAN_FILEINFO_XL_BITRATE,
			pretty_unit(pci->xl_bitrate, tmp, PCANINFO_MAX_CHAR_SIZE));
		separator++;
	}
	...
	if (pci->availflag_xl & PCANINFO_FLAG_XL_ES) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_ES, pci->xl_es);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_PWM_LONG) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_PWM_LONG, pci->xl_pwm_long);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_PWM_OFFSET) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_PWM_OFFSET, pci->xl_pwm_offset);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_PWM_SHORT) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_PWM_SHORT, pci->xl_pwm_short);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_SJW) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_SJW, pci->xl_sjw);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_SP) {
		fprintf(stdout, "  \t- %s: %.02f%%\n", PCAN_FILEINFO_XL_SP, pci->xl_sp/100.0f);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_SSP_OFFSET) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_SSP, pci->xl_ssp_offset);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_TMS) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_TMS, pci->xl_tms);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_TQ) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_TQ, pci->xl_tq);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_TSEG1) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_TSEG1, pci->xl_tseg1);
		separator++;
	}
	if (pci->availflag_xl & PCANINFO_FLAG_XL_TSEG2) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_XL_TSEG2, pci->xl_tseg2);
		separator++;
	}
	...
	if (pci->availflag & PCANINFO_FLAG_RXERR) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_RX_ERRORS, pci->rxerr);
		separator++;
	}
	if (pci->availflag & PCANINFO_FLAG_TXERR) {
		fprintf(stdout, "  \t- %s: %d\n", PCAN_FILEINFO_TX_ERRORS, pci->txerr);
		separator++;
	}
...
void pcaninfo_output_summary(struct pcaninfo * pci, char* handle_info) {
	...
	if (pci->bus_state != 0)
	{
		if (pci->availflag & PCANINFO_FLAG_NOM_BITRATE) {
			...
			/* append xl bitrate if bus is initialized as CAN XL */
			if ((pci->init_flags & PCANXL_INIT_XL) == PCANXL_INIT_XL) {
				if (pci->availflag & PCANINFO_FLAG_XL_BITRATE) {
					fprintf(stdout, " | %sBit/s",
							pretty_unit(pci->xl_bitrate, tmp, PCANINFO_MAX_CHAR_SIZE));
				}
			}
...
int pcaninfo_driver_version(char *buffer, unsigned int size) {
	...
	else
	{
		...
		read = getline(&line, &len, f);
		if (read != -1) {
			lenver = MIN(size, (unsigned int)read);
...
char* pcaninfo_bitrate_to_init_string(struct pcaninfo * pci, char *buffer, unsigned int size) {
	...
	/* handle non CAN-XL initialization */
	if ((pci->init_flags & PCANXL_INIT_XL) == 0)
	{
	if ((pci->availflag_ex & PCANINFO_FLAG_EX_NOM_BRP)) {
	...
	if ((pci->availflag_ex & PCANINFO_FLAG_EX_DATA_SJW)) {
		...
		if ((pci->availflag_ex & PCANINFO_FLAG_EX_NOM_BRP)) {
			snprintf(tmpValue, sizeof(tmpValue), "brp=%d,", pci->nom_brp);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_EX_NOM_TSEG1)) {
			snprintf(tmpValue, sizeof(tmpValue), "nom_tseg1=%d,", pci->nom_tseg1);
		...
		if ((pci->availflag_ex & PCANINFO_FLAG_EX_DATA_TSEG1)) {
			snprintf(tmpValue, sizeof(tmpValue), "fd_tseg1=%d,", pci->data_tseg1);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_EX_DATA_TSEG2)) {
			snprintf(tmpValue, sizeof(tmpValue), "fd_tseg2=%d,", pci->data_tseg2);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_EX_DATA_SJW)) {
			snprintf(tmpValue, sizeof(tmpValue), "fd_sjw=%d,", pci->data_sjw);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_EX_DATA_SAMPLE_POINT)) {
			snprintf(tmpValue, sizeof(tmpValue), "fd_ssp_offset=%d,", pci->data_sample_point);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_XL_TSEG1)) {
			snprintf(tmpValue, sizeof(tmpValue), "xl_tseg1=%d,", pci->xl_tseg1);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_XL_TSEG2)) {
			snprintf(tmpValue, sizeof(tmpValue), "xl_tseg2=%d,", pci->xl_tseg2);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_XL_SJW)) {
			snprintf(tmpValue, sizeof(tmpValue), "xl_sjw=%d,", pci->xl_sjw);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_XL_SP)) {
			snprintf(tmpValue, sizeof(tmpValue), "xl_ssp_offset=%d,", pci->xl_sp);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_XL_ES)) {
			snprintf(tmpValue, sizeof(tmpValue), "xl_error_signaling=%d,", pci->xl_es);
			strcat(buffer, tmpValue);
		}
		if ((pci->availflag_ex & PCANINFO_FLAG_XL_TMS)) {
			snprintf(tmpValue, sizeof(tmpValue), "xl_transceiver_mode_switch=%d,", pci->xl_tms);
			strcat(buffer, tmpValue);
		}
	}
	return buffer;
